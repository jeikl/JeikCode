...
[Sign in](https://ai.google.dev/_d/signin?continue=https%3A%2F%2Fai.google.dev%2Fapi%2Fgenerate-content&prompt=select_account)
...
- [Model API](https://ai.google.dev/api/models)
- [Agents](https://ai.google.dev/api/agents)
- [Webhooks](https://ai.google.dev/api/webhooks)
- [Triggers](https://ai.google.dev/api/triggers)
- [Environments](https://ai.google.dev/api/environments)
...
- File Search API
  ...
  - [Documents](https://ai.google.dev/api/file-search/documents)
...
- [Content](https://ai.google.dev/api/generate-content#content)
...
- [NetworkMode](https://ai.google.dev/api/generate-content#networkmode)
- [Schema](https://ai.google.dev/api/generate-content#schema)
...
- [REST Resource: auth\_tokens](https://ai.google.dev/api/generate-content#rest-resource:-auth_tokens)
...
- [Method: auth\_tokens.create](https://ai.google.dev/api/generate-content#method:-auth_tokens.create)
  - [Endpoint](https://ai.google.dev/api/generate-content#endpoint_2)
  - [Request body](https://ai.google.dev/api/generate-content#request-body_2)
  - [Response body](https://ai.google.dev/api/generate-content#response-body_2)
...
| `UNSTABLE_EXPERIMENTAL` | The underlying model is subject to lots of tunings.<br>This item is deprecated! |
...
| `DEPRECATED` | Models in this stage are deprecated. These models cannot be used.<br>This item is deprecated! |
...
Chunk type. The following is a list of mutually exclusive fields. At most one of the fields will be set in a response:
...
Optional. The media blob resource name for multimodal file search results. Format: fileSearchStores/{file\_search\_store\_id}/media/{blobId}

| JSON representation |
| --- |
| ```<br>{<br>  "customMetadata": [<br>    {<br>      object (CustomMetadata)<br>    }<br>  ],<br>  "uri": string,<br>  "title": string,<br>  "text": string,<br>  "fileSearchStore": string,<br>  "pageNumber": integer,<br>  "mediaId": string<br>}<br>``` |
...
The value of the metadata. Can be a string, a list of strings, or a number. The following is a list of mutually exclusive fields. At most one of the fields will be set in a response:
...
| `HARM_CATEGORY_CIVIC_INTEGRITY` | **Gemini** \- Content that may be used to harm civic integrity. DEPRECATED: use enableEnhancedCivicAnswers instead.<br>This item is deprecated! |
...
| JSON representation |
| --- |
| ```<br>{<br>  "parts": [<br>    {<br>      object (Part)<br>    }<br>  ],<br>  "role": string<br>}<br>``` |
...
Controls extra preprocessing of data. The following is a list of mutually exclusive fields. At most one of the fields will be set in a response:
...
| JSON representation |
| --- |
| ```<br>{<br>  "thought": boolean,<br>  "thoughtSignature": string,<br>  "partMetadata": {<br>    object<br>  },<br>  "mediaResolution": {<br>    object (MediaResolution)<br>  },<br>  "mediaProcessing": enum (MediaProcessing),<br>  // data<br>  "text": string,<br>  "inlineData": {<br>    object (Blob)<br>  },<br>  "functionCall": {<br>    object (FunctionCall)<br>  },<br>  "functionResponse": {<br>    object (FunctionResponse)<br>  },<br>  "fileData": {<br>    object (FileData)<br>  },<br>  "executableCode": {<br>    object (ExecutableCode)<br>  },<br>  "codeExecutionResult": {<br>    object (CodeExecutionResult)<br>  },<br>  "toolCall": {<br>    object (ToolCall)<br>  },<br>  "toolResponse": {<br>    object (ToolResponse)<br>  }<br>  // Union type<br>  // metadata<br>  "videoMetadata": {<br>    object (VideoMetadata)<br>  }<br>  // Union type<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "mimeType": string,<br>  "data": string<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "id": string,<br>  "name": string,<br>  "args": {<br>    object<br>  }<br>}<br>``` |
...
Multimedia can be included by using a subobject containing a single "$ref" key whose value is the `inlineData.display_name` of a `FunctionResponsePart` holding the multimedia. See [https://ai.google.dev/gemini-api/docs/function-calling#multimodal](https://ai.google.dev/gemini-api/docs/function-calling#multimodal).
...
Optional. Signals that function call continues, and more responses will be returned, turning the function call into a generator. Is only applicable to NON\_BLOCKING function calls, is ignored otherwise. If set to false, future responses will not be considered. It is allowed to return empty `response` with `willContinue=False` to signal that the function call is finished. This may still trigger the model generation. To avoid triggering the generation and finish the function call, additionally set `scheduling` to `SILENT`.

`scheduling``enum (Scheduling)`

Optional. Specifies how the response should be scheduled in the conversation. Only applicable to NON\_BLOCKING function calls, is ignored otherwise. Defaults to WHEN\_IDLE.

| JSON representation |
| --- |
| ```<br>{<br>  "id": string,<br>  "name": string,<br>  "response": {<br>    object<br>  },<br>  "parts": [<br>    {<br>      object (FunctionResponsePart)<br>    }<br>  ],<br>  "willContinue": boolean,<br>  "scheduling": enum (Scheduling)<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  // data<br>  "inlineData": {<br>    object (FunctionResponseBlob)<br>  }<br>  // Union type<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "mimeType": string,<br>  "fileUri": string<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "id": string,<br>  "language": enum (Language),<br>  "code": string<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "id": string,<br>  "outcome": enum (Outcome),<br>  "output": string<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "id": string,<br>  "toolName": string,<br>  "toolType": enum (ToolType),<br>  "args": {<br>    object<br>  }<br>}<br>``` |
...
| `GOOGLE_SEARCH_WEB` | Google search tool, maps to Tool.google\_search.search\_types.web\_search. |
| `GOOGLE_SEARCH_IMAGE` | Image search tool, maps to Tool.google\_search.search\_types.image\_search. |
| `URL_CONTEXT` | URL context tool, maps to Tool.url\_context. |
| `GOOGLE_MAPS` | Google maps tool, maps to Tool.google\_maps. |
| `FILE_SEARCH` | File search tool, maps to Tool.file\_search. |
...
| JSON representation |
| --- |
| ```<br>{<br>  "id": string,<br>  "toolType": enum (ToolType),<br>  "response": {<br>    object<br>  }<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "startOffset": string,<br>  "endOffset": string,<br>  "fps": number<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  // value<br>  "level": enum (Level)<br>  // Union type<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "id": string,<br>  "sources": [<br>    {<br>      object (Source)<br>    }<br>  ],<br>  "created": string,<br>  "updated": string,<br>  "lastAccessed": string,<br>  "status": enum (Status),<br>  "fileCount": string,<br>  "sizeBytes": string,<br>  // network<br>  "networkAllowlist": {<br>    object (EnvironmentNetworkEgressAllowlist)<br>  },<br>  "networkMode": enum (NetworkMode)<br>  // Union type<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "type": enum (Type),<br>  "format": string,<br>  "title": string,<br>  "description": string,<br>  "nullable": boolean,<br>  "enum": [<br>    string<br>  ],<br>  "maxItems": string,<br>  "minItems": string,<br>  "properties": {<br>    string: {<br>      object (Schema)<br>    },<br>    ...<br>  },<br>  "required": [<br>    string<br>  ],<br>  "minProperties": string,<br>  "maxProperties": string,<br>  "minLength": string,<br>  "maxLength": string,<br>  "pattern": string,<br>  "example": value,<br>  "anyOf": [<br>    {<br>      object (Schema)<br>    }<br>  ],<br>  "propertyOrdering": [<br>    string<br>  ],<br>  "default": value,<br>  "items": {<br>    object (Schema)<br>  },<br>  "minimum": number,<br>  "maximum": number<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "functionDeclarations": [<br>    {<br>      object (FunctionDeclaration)<br>    }<br>  ],<br>  "googleSearchRetrieval": {<br>    object (GoogleSearchRetrieval)<br>  },<br>  "codeExecution": {<br>    object (CodeExecution)<br>  },<br>  "googleSearch": {<br>    object (GoogleSearch)<br>  },<br>  "computerUse": {<br>    object (ComputerUse)<br>  },<br>  "urlContext": {<br>    object (UrlContext)<br>  },<br>  "fileSearch": {<br>    object (FileSearch)<br>  },<br>  "mcpServers": [<br>    {<br>      object (McpServer)<br>    }<br>  ],<br>  "googleMaps": {<br>    object (GoogleMaps)<br>  }<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "name": string,<br>  "description": string,<br>  "behavior": enum (Behavior),<br>  "parameters": {<br>    object (Schema)<br>  },<br>  "parametersJsonSchema": value,<br>  "response": {<br>    object (Schema)<br>  },<br>  "responseJsonSchema": value<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "dynamicRetrievalConfig": {<br>    object (DynamicRetrievalConfig)<br>  }<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "mode": enum (Mode),<br>  "dynamicThreshold": number<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "timeRangeFilter": {<br>    object (Interval)<br>  },<br>  "searchTypes": {<br>    object (SearchTypes)<br>  }<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "startTime": string,<br>  "endTime": string<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "webSearch": {<br>    object (WebSearch)<br>  },<br>  "imageSearch": {<br>    object (ImageSearch)<br>  }<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "environment": enum (Environment),<br>  "excludedPredefinedFunctions": [<br>    string<br>  ],<br>  "enablePromptInjectionDetection": boolean,<br>  "disabledSafetyPolicies": [<br>    enum (SafetyPolicy)<br>  ]<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "fileSearchStoreNames": [<br>    string<br>  ],<br>  "metadataFilter": string,<br>  "topK": integer<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "name": string,<br>  // transport<br>  "streamableHttpTransport": {<br>    object (StreamableHttpTransport)<br>  }<br>  // Union type<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "url": string,<br>  "headers": {<br>    string: string,<br>    ...<br>  },<br>  "timeout": string,<br>  "sseReadTimeout": string,<br>  "terminateOnClose": boolean<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "enableWidget": boolean<br>}<br>``` |

## REST Resource: auth\_tokens
...
If fieldMask is empty, and `bidiGenerateContentSetup` _is_ present, then the effective `BidiGenerateContentSetup` message is taken entirely from `bidiGenerateContentSetup` in this request. The setup message from the Live API connection is ignored.
...
| JSON representation |
| --- |
| ```<br>{<br>  "name": string,<br>  "expireTime": string,<br>  "newSessionExpireTime": string,<br>  "fieldMask": string,<br>  // config<br>  "bidiGenerateContentSetup": {<br>    object (BidiGenerateContentSetup)<br>  }<br>  // Union type<br>  "uses": integer<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "model": string,<br>  "generationConfig": {<br>    object (GenerationConfig)<br>  },<br>  "systemInstruction": {<br>    object (Content)<br>  },<br>  "tools": [<br>    {<br>      object (Tool)<br>    }<br>  ],<br>  "realtimeInputConfig": {<br>    object (RealtimeInputConfig)<br>  },<br>  "sessionResumption": {<br>    object (SessionResumptionConfig)<br>  },<br>  "contextWindowCompression": {<br>    object (ContextWindowCompressionConfig)<br>  },<br>  "inputAudioTranscription": {<br>    object (AudioTranscriptionConfig)<br>  },<br>  "outputAudioTranscription": {<br>    object (AudioTranscriptionConfig)<br>  },<br>  "historyConfig": {<br>    object (HistoryConfig)<br>  }<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "text": {<br>    object (TextResponseFormat)<br>  },<br>  "audio": {<br>    object (AudioResponseFormat)<br>  },<br>  "image": {<br>    object (ImageResponseFormat)<br>  }<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "mimeType": enum (MimeType),<br>  "schema": value<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "mimeType": enum (MimeType),<br>  "delivery": enum (Delivery),<br>  "sampleRate": integer,<br>  "bitRate": integer<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "mimeType": enum (MimeType),<br>  "delivery": enum (Delivery),<br>  "aspectRatio": enum (AspectRatio),<br>  "imageSize": enum (ImageSize)<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "targetLanguageCode": string,<br>  "echoTargetLanguage": boolean<br>}<br>``` |
...
`languageAuto
(deprecated)``object (LanguageAuto)`
...
`languageHints
(deprecated)``object (LanguageHints)`
...
| JSON representation |
| --- |
| ```<br>{<br>  "languageCodes": [<br>    string<br>  ],<br>  "adaptationPhrases": [<br>    string<br>  ],<br>  "customVocabulary": [<br>    string<br>  ],<br>  "wordTimestamp": boolean,<br>  "diarization": boolean,<br>  "mode": enum (Mode),<br>  // language_config<br>  "languageAuto": {<br>    object (LanguageAuto)<br>  },<br>  "languageHints": {<br>    object (LanguageHints)<br>  }<br>  // Union type<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "languageCodes": [<br>    string<br>  ]<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "automaticActivityDetection": {<br>    object (AutomaticActivityDetection)<br>  },<br>  "activityHandling": enum (ActivityHandling),<br>  "turnCoverage": enum (TurnCoverage)<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "disabled": boolean,<br>  "startOfSpeechSensitivity": enum (StartSensitivity),<br>  "prefixPaddingMs": integer,<br>  "endOfSpeechSensitivity": enum (EndSensitivity),<br>  "silenceDurationMs": integer<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  // compression_mechanism<br>  "slidingWindow": {<br>    object (SlidingWindow)<br>  }<br>  // Union type<br>  "triggerTokens": string<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "targetTokens": string<br>}<br>``` |
...
| JSON representation |
| --- |
| ```<br>{<br>  "initialHistoryInClientContent": boolean<br>}<br>``` |

## Method: auth\_tokens.create
...
- [Manage cookies](https://ai.google.dev/api/generate-content#)
