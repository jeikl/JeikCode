...
**Files:** Modify `crates/jeikcode-core/src/{process_utils.rs, stream/mod.rs, tool/mod.rs, skill.rs, graph/indexer.rs, plugin/installer.rs}`。

- [ ] **Step 1: 搬 `real_home_dir` 到 process_utils**
  - 从 `tool/mod.rs`（~360）剪切 `pub fn real_home_dir() -> Option<PathBuf>` 全体（连同其私有 helper，若有）到 `crates/jeikcode-core/src/process_utils.rs`（加必要的 `use std::path::PathBuf;` 等）。若 tool/mod.rs 有 `real_home_dir` 的单测，一并搬。
...
- [ ] **Step 2: 搬 `ToolCall` 到 core::stream**
  - 从 `tool/mod.rs`（~824）剪切 `pub struct ToolCall { pub id: String, pub name: String, pub arguments: String }`（连其 derive 属性）到 `crates/jeikcode-core/src/stream/mod.rs`。⚠️不要搬 `ToolCallBuffer`（那是 tool 内部，随球删）。
...
**Files:** Modify `crates/jeikcode-capabilities/src/tools/approval.rs` + `tools/mod.rs`（re-export）；`crates/jeikcode-daemon/src/{permission_bridge.rs, lib.rs, live_api.rs}`。
...
- [ ] **Step 1: 写失败测试（capabilities parse_permission_decision）**
  在 `crates/jeikcode-capabilities/src/tools/approval.rs` 的 `#[cfg(test)]`：
  ```rust
  #[test]
...
- [ ] **Step 2: 跑测试确认失败** → `cargo test -p jeikcode-capabilities parse_permission_decision`（未定义）。
- [ ] **Step 3: 实现**
  approval.rs 加：
  ```rust
...
**Files:** Delete `crates/jeikcode-core/src/{tool/, conversation/, provider/, ctx/}`；Modify `crates/jeikcode-core/src/lib.rs`；Delete orphan tests。
...
- [ ] **Step 2: 删模块**
  ```bash
  git rm -r crates/jeikcode-core/src/tool crates/jeikcode-core/src/conversation crates/jeikcode-core/src/provider crates/jeikcode-core/src/ctx
  ```
...
- [ ] **Step 6: 提交**
  ```bash
  git add -A crates/jeikcode-core/
