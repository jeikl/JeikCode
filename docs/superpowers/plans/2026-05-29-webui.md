# jeikcode webui (Phase 1) Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 让用户在 TUI 输入 `/webui`（或命令行 `jeikcode webui`）即可在本地浏览器中聊天、运行 agent、交互式批准工具调用——作为 TUI 之外的并行入口。

**Architecture:** 把 `jeikcode-daemon` 的服务逻辑下沉为库函数 `run_server(...)`（原二进制变薄壳），`jeikcode` 主程序依赖它，`/webui` 在**进程内** `tokio::spawn(run_server)` 起服务——不 spawn 子进程、不依赖独立 daemon 二进制（普通用户 `curl|sh` 只装主程序）。前端用 Preact+Vite+Tailwind 构建为静态资源，经 `rust-embed` 嵌入并由 `run_server` 在 `/` 提供。`/chat` 把权限从 `BypassAll` 换成已有的 `InteractivePermissionDecider`，SSE 推 `permission_request`、新增 `POST /chat/permission` 回送决定。每会话独立 `working_dir`（随 `/chat` 请求带），新增 `GET /fs/list` 供目录浏览。`/webui` 命令：起 server（若未起）+ mint token + 开浏览器。Phase 2 官方中转隧道仅留可插拔鉴权接口。

**重要前提（已核实）：** `jeikcode` 已是 `#[tokio::main]`（CLI `main.rs:698`），依赖含 `tokio(full)`+`reqwest`；`install.sh` 每平台只下载主程序、不含 daemon；`jeikcode-cli` 原本不依赖 daemon crate。故 server 必须库化供主程序进程内调用。

**Tech Stack:** Rust / axum 0.7 / tokio / rust-embed；前端 Preact + Vite + Tailwind；复用 `jeikcode-core` 的 `InteractivePermissionDecider`、`TurnRunner`、`open_browser`。
...
- `crates/jeikcode-daemon/src/webui.rs`：rust-embed 静态资源 handler + SPA fallback
- `crates/jeikcode-daemon/src/auth_token.rs`：一次性 token 存储 + 鉴权中间件
- `crates/jeikcode-daemon/src/permission_bridge.rs`：把 daemon `/chat` 的待批工具桥接到 SSE + `/chat/permission`

**修改：**
- `crates/jeikcode-daemon/src/lib.rs`（**新建**）：暴露 `pub async fn run_server(opts: ServerOpts) -> anyhow::Result<()>` 与 `pub fn build_app_state(...) -> AppState`、`pub use` token/权限类型；把 `main.rs` 中 Router 构建 + bind/serve 主体迁进来
- `crates/jeikcode-daemon/src/main.rs`：变薄壳，解析参数后调 `jeikcode_daemon::run_server(...)`
- `crates/jeikcode-daemon/Cargo.toml`：加 `[lib]`、`rust-embed` 依赖；加 feature `webui`（默认开）
- `crates/jeikcode-cli/Cargo.toml`：加依赖 `jeikcode-daemon = { path = "../jeikcode-daemon" }`
- `crates/jeikcode-cli/src/main.rs`：`Commands` 加 `Webui` 子命令；进程内 server 单例
- `crates/jeikcode-cli/src/webui_launch.rs`（**新建**）：`ensure_server_and_open(port) -> Result<String>` —— 进程内起 server（`OnceCell` 守护）+ mint token + 开浏览器，TUI 与 CLI 共用
- `crates/jeikcode-tuix/src/event_loop/commands.rs`：`match cmd` 加 `"webui"` 分支（调 cli 暴露的 launcher，或把 launcher 放 core 供 tuix 调）

> **launcher 归属**：tuix 与 cli 都要用。放 `jeikcode-cli` 会让 tuix 反向依赖 cli（不可）。故把 `ensure_server_and_open` 放 **`jeikcode-core`**（core 可依赖 daemon 库吗？会成环：daemon 依赖 core）。**结论：放 `jeikcode-daemon` 库里**（`pub fn ensure_server_and_open`），core/tuix/cli 都能依赖 daemon 库调它。tuix 已可依赖 daemon 库（新增依赖）。实现时确认无依赖环（daemon→core 单向，tuix→daemon 新增，cli→daemon 新增，均不成环）。

**关键既有接口（已核实，实现时直接用）：**
- `jeikcode_core::turn::permission::{InteractivePermissionDecider, ApprovalRequest}`
...
- `jeikcode_core::tool::{PermissionDecision (Allow|Ask(String)|Deny), PermissionStore, ToolCall}`
- `jeikcode_core::turn::event::TurnEvent::ApprovalRequested { tool_name, reason, call, messages }`（daemon SSE 循环当前在 `main.rs:2299` 显式丢弃它）
- `jeikcode_core::auth::oauth::open_browser(url: &str) -> anyhow::Result<()>`（已按平台 cfg）
- daemon 现有：`origin_is_allowed(&str) -> bool`、`is_loopback_authority(&str) -> bool`、`activity_tracker_middleware`
- daemon 默认 `127.0.0.1:13456`；CLI `Commands::Daemon` 通过 re-exec `jeikcode-daemon` 二进制启动（`jeikcode-cli/src/main.rs:930+`）
...
**Files:**
- Create: `crates/jeikcode-daemon/src/lib.rs`
- Modify: `crates/jeikcode-daemon/src/main.rs`、`crates/jeikcode-daemon/Cargo.toml`

- [ ] **Step 1: 加 `[lib]` 段**

`crates/jeikcode-daemon/Cargo.toml` 在 `[package]` 后加：

```toml
[lib]
name = "jeikcode_daemon"
path = "src/lib.rs"

[[bin]]
name = "jeikcode-daemon"
path = "src/main.rs"
```
...
```rust
//! jeikcode-daemon 既是独立二进制，也是库：`run_server` 供主程序进程内调用，
//! 使 `/webui` 无需第二个二进制。
...
async fn main() {
    ...
    if let Err(e) = jeikcode_daemon::run_server(jeikcode_daemon::ServerOpts {
...
Run: `cargo build -p jeikcode-daemon && cargo test -p jeikcode-daemon`
Expected: 二进制与库都编译通过，既有 daemon 测试全过（行为不变）。

- [ ] **Step 5: 冒烟测试（行为不变）**

```bash
cargo run -p jeikcode-daemon -- --port 13457 &
sleep 2
...
```bash
git add crates/jeikcode-daemon
...
> 在库化后这些已迁入 `crates/jeikcode-daemon/src/lib.rs`，请在 `lib.rs` 中改动；
> `mod xxx;` 声明也加在 `lib.rs`。`crate::AppState` 引用在 lib 内同样有效。

### Task 1: 一次性 token 存储 + 校验逻辑（纯函数，可单测）

**Files:**
- Create: `crates/jeikcode-daemon/src/auth_token.rs`
- Modify: `crates/jeikcode-daemon/src/main.rs`（声明 `mod auth_token;`，AppState 加字段）

- [ ] **Step 1: 写失败测试**

在 `crates/jeikcode-daemon/src/auth_token.rs` 底部：
...
Run: `cargo test -p jeikcode-daemon auth_token`
Expected: 编译失败（`WebuiTokenStore` 未定义）。

- [ ] **Step 3: 实现 token 存储**

在 `crates/jeikcode-daemon/src/auth_token.rs` 顶部：
...
```bash
git add crates/jeikcode-daemon/src/auth_token.rs crates/jeikcode-daemon/src/main.rs
...
**Files:**
- Modify: `crates/jeikcode-daemon/src/auth_token.rs`（加中间件函数 + 测试）
- Modify: `crates/jeikcode-daemon/src/main.rs`（AppState 加 `webui_tokens` 字段；初始化；Router 套层）
...
Run: `cargo test -p jeikcode-daemon auth_token && cargo build -p jeikcode-daemon`
...
**Files:**
- Modify: `crates/jeikcode-daemon/Cargo.toml`
- Create: `webui/dist/index.html`（占位，后续被真实构建覆盖）

- [ ] **Step 1: 加依赖**

在 `crates/jeikcode-daemon/Cargo.toml` 的 `[dependencies]` 加：
...
```html
<!doctype html>
<html><head><meta charset="utf-8"><title>jeikcode webui</title></head>
<body><div id="app">webui placeholder</div></body></html>
```

- [ ] **Step 3: 编译确认依赖可用**

Run: `cargo build -p jeikcode-daemon`
Expected: 成功（仅引入了依赖，未使用尚不报错）。

- [ ] **Step 4: 提交**

```bash
git add crates/jeikcode-daemon/Cargo.toml webui/dist/index.html
...
**Files:**
- Create: `crates/jeikcode-daemon/src/webui.rs`
- Modify: `crates/jeikcode-daemon/src/main.rs`（`mod webui;`，Router 加 `GET /` 与 fallback）

- [ ] **Step 1: 写 handler 测试**

`crates/jeikcode-daemon/src/webui.rs` 底部：
...
Run: `cargo test -p jeikcode-daemon webui`
Expected: 编译失败（`WebuiAssets` / `asset_or_index` 未定义）。

- [ ] **Step 3: 实现 embed + handler**

`crates/jeikcode-daemon/src/webui.rs` 顶部：
...
Run: `cargo test -p jeikcode-daemon webui && cargo build -p jeikcode-daemon`
...
```bash
git add crates/jeikcode-daemon/src/webui.rs crates/jeikcode-daemon/src/main.rs
...
**Files:**
- Create: `crates/jeikcode-daemon/src/permission_bridge.rs`
- Modify: `crates/jeikcode-daemon/src/main.rs`（AppState 加 `pending_permissions`）
...
mod tests {
    use super::*;
    use jeikcode_core::tool::PermissionDecision;
...
Run: `cargo test -p jeikcode-daemon permission_bridge`
...
Run: `cargo test -p jeikcode-daemon permission_bridge && cargo build -p jeikcode-daemon`
Expected: PASS + 编译通过。

- [ ] **Step 5: 提交**

```bash
git add crates/jeikcode-daemon/src/permission_bridge.rs crates/jeikcode-daemon/src/main.rs
...
**Files:**
- Modify: `crates/jeikcode-daemon/src/main.rs`（`chat_stream` 内 permission 构造处 `main.rs:2051`；SSE 循环 `main.rs:2299`）
...
```rust
    ...
    use jeikcode_core::turn::permission::{ApprovalRequest, InteractivePermissionDecider};
    use jeikcode_core::tool::PermissionStore;
    let (perm_req_tx, mut perm_req_rx) =
        tokio::sync::mpsc::unbounded_channel::<ApprovalRequest>();
    let (perm_resp_tx, perm_resp_rx) =
        tokio::sync::mpsc::unbounded_channel::<jeikcode_core::tool::PermissionDecision>();
...
```bash
git add crates/jeikcode-daemon/src/main.rs
...
**Files:**
- Modify: `crates/jeikcode-daemon/src/main.rs`（新 handler + 路由）
...
**Files:**
- Modify: `crates/jeikcode-daemon/src/main.rs`（给 `/chat`、`/chat/permission`、`/sessions` 等数据路由套 `require_webui_token`）
...
**Files:**
- Modify: `crates/jeikcode-daemon/src/lib.rs`（新增 `ensure_server_and_open` + 进程内单例 + `mint_token` 直调）
...
/// 在 `jeikcode` 主程序（已有 tokio runtime）内调用。
pub async fn ensure_server_and_open(port: u16) -> String {
    ...
    match jeikcode_core::auth::oauth::open_browser(&url) {
...
**Files:**
- Modify: `crates/jeikcode-tuix/Cargo.toml`（加 `jeikcode-daemon = { path = "../jeikcode-daemon" }`）
- Modify: `crates/jeikcode-tuix/src/event_loop/commands.rs`（`match cmd` 加 `"webui"`）
- Modify: `crates/jeikcode-tuix/src/commands.rs`（注册命令 + i18n）

- [ ] **Step 1: tuix 依赖 daemon 库**

`crates/jeikcode-tuix/Cargo.toml` 的 `[dependencies]` 加 `jeikcode-daemon = { path = "../jeikcode-daemon" }`。
确认无依赖环（daemon→core 单向；tuix→daemon 新增；不成环）。`cargo build -p jeikcode-tuix` 验证。
...
```rust
        "webui" => {
            // tuix 已在 tokio runtime 内（jeikcode_tuix::run 是 async）。
            let msg = jeikcode_daemon::ensure_server_and_open(13456).await;
...
在 `crates/jeikcode-tuix/src/commands.rs`（`CommandRegistry::builtin`）注册 `webui`，中英描述：中文「启动浏览器 webui」，英文 "Launch the browser webui"（参照已有命令的 `Msg::` 条目；i18n 文件 `crates/jeikcode-core/src/i18n/{en,zh_cn,messages}.rs`）。
...
```bash
git add crates/jeikcode-tuix/Cargo.toml crates/jeikcode-tuix/src/event_loop/commands.rs crates/jeikcode-tuix/src/commands.rs crates/jeikcode-core/src/i18n
git commit -m "feat(tuix): /webui 命令（进程内起 server + 开浏览器）"
```

### Task 10: `jeikcode webui` CLI 子命令

**Files:**
- Modify: `crates/jeikcode-cli/Cargo.toml`（加 `jeikcode-daemon` 依赖）
- Modify: `crates/jeikcode-cli/src/main.rs`（`Commands` 加 `Webui`）

- [ ] **Step 1: cli 依赖 daemon 库**

`crates/jeikcode-cli/Cargo.toml` 加 `jeikcode-daemon = { path = "../jeikcode-daemon" }`。
...
```rust
            Commands::Webui { port } => {
                ...
                let msg = jeikcode_daemon::ensure_server_and_open(port).await;
                eprintln!("{msg}");
...
Run: `cargo build -p jeikcode-cli`
手测：`cargo run -p jeikcode-cli -- webui`，应进程内起 server 并开浏览器占位页。
Expected: 浏览器打开占位页；Ctrl+C 退出。

- [ ] **Step 5: 提交**

```bash
git add crates/jeikcode-cli/Cargo.toml crates/jeikcode-cli/src/main.rs
git commit -m "feat(cli): jeikcode webui 子命令（进程内）"
```
...
> 前端不走 TDD（无单测基建），以「构建通过 + 手动验证清单」为准。每个 Task 末尾 `npm run build` 产物落到 `webui/dist/`（Task 4 的 embed 源目录），随后 `cargo build -p jeikcode-daemon` 重新嵌入。
...
<html lang="zh">
  ...
  <title>jeikcode webui</title></head>
...
```tsx
export function App() {
  return <div class="p-4 text-lg">jeikcode webui — loading…</div>;
}
```
...
Run: `cd webui && npm run build && cd .. && cargo build -p jeikcode-daemon`
Expected: `webui/dist/` 生成，daemon 编译通过。

- [ ] **Step 4: 手测**

`cargo run -p jeikcode-daemon -- --port 13457`，浏览器开 `127.0.0.1:13457/`，应显示 "jeikcode webui — loading…"。

- [ ] **Step 5: 提交**

```bash
git add webui/ crates/jeikcode-daemon
...
**Files:**
- Modify: `crates/jeikcode-daemon/src/lib.rs`（新 handler + 路由）
...
mod fs_list_tests {
    ...
    #[test]
    fn expands_tilde() {
        let home = jeikcode_core::tool::real_home_dir().unwrap();
...
Run: `cargo test -p jeikcode-daemon fs_list`
...
pub fn normalize_dir_arg(arg: &str) -> PathBuf {
    if let Some(rest) = arg.strip_prefix('~') {
        if let Some(home) = jeikcode_core::tool::real_home_dir() {
...
Run: `cargo test -p jeikcode-daemon fs_list && cargo build -p jeikcode-daemon`
...
**Files:**
- Modify: `crates/jeikcode-tuix/src/event_loop/commands.rs`（`webui` 分支支持 `stop` 参数）
- Modify: `crates/jeikcode-daemon/src/webui.rs`（`JEIKCODE_WEBUI_DEV` 重定向到 vite dev server）
...
`webui::serve_webui` 开头：若 `std::env::var("JEIKCODE_WEBUI_DEV")` 存在，则 302 重定向到该地址 + 原 path，便于前端热更新开发。

- [ ] **Step 3: 文档**

`webui/README.md` 写：开发 `npm run dev` + 设 `JEIKCODE_WEBUI_DEV`；发布 `npm run build` 后 `cargo build`。主 README 加 `/webui` 用法与「作为 TUI 之外入口」的定位说明。

- [ ] **Step 4: 构建 + 全量编译 + 手测**

Run: `cd webui && npm run build && cd .. && cargo build && cargo test -p jeikcode-daemon`
手测：`/webui stop` 能停进程内 server；设 `JEIKCODE_WEBUI_DEV=http://localhost:5173` 后访问根路径重定向到 vite。
Expected: 均正常。
...
`ls scripts/`：在打包 **主程序 `jeikcode`** 的脚本（`release.sh`/`*-release-*.sh`）里，于 `cargo build --release` 之前插入 `(cd webui && npm ci && npm run build)`，保证嵌入主程序的是最新前端。
> 关键：前端现在嵌入的是 **`jeikcode` 主程序**（经 daemon 库 `run_server` 服务），不再只是 daemon 二进制——主程序的发布管线必须包含前端构建。
...
**Files:**
- Create: `crates/jeikcode-core/src/turn/live_session.rs`
- Modify: `crates/jeikcode-core/src/turn/mod.rs`（`pub mod live_session;`）
...
Run: `cargo test -p jeikcode-core live_session`
...
```bash
git add crates/jeikcode-core/src/turn/live_session.rs crates/jeikcode-core/src/turn/mod.rs
...
**Files:**
- Modify: `crates/jeikcode-core/src/turn/live_session.rs`（加 `run_coordinator`）
...
```bash
git add crates/jeikcode-core/src/turn/live_session.rs
...
**Files:**
- Modify: `crates/jeikcode-daemon/src/lib.rs`（AppState 持有 `Option<LiveSession>`；新路由）

- [ ] **Step 1: AppState 持有 LiveSession + 启动协调器**

AppState 加 `pub live: Arc<RwLock<Option<jeikcode_core::turn::live_session::LiveSession>>>`。
...
async fn live_message(State(state): State<AppState>, Json(req): Json<ChatRequest>) -> impl IntoResponse {
    if let Some(live) = state.live.read().await.clone() {
        ...
        live.submit(jeikcode_core::turn::live_session::UserInput {
...
**Files:**
- Modify: `crates/jeikcode-tuix/src/event_loop/mod.rs`、`commands.rs`
...
```bash
git add crates/jeikcode-tuix/src
...
**Files:**
- Modify: `crates/jeikcode-core/src/turn/live_session.rs` 或协调器；`crates/jeikcode-tuix`、`webui`
...
- **分发修复**：原计划假设 `jeikcode-daemon` 二进制随主程序分发（实际 install.sh 只装主程序）——
