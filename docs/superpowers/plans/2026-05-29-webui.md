...
`cargo run -p jeikcode-daemon -- --port 13457`，浏览器开 `127.0.0.1:13457/`，应显示 "atomcode webui — loading…"。
...
- Modify: `crates/jeikcode-daemon/src/webui.rs`（`ATOMCODE_WEBUI_DEV` 重定向到 vite dev server）
