...
> 目标：把 `core::vision_preprocessor::maybe_preprocess`（用 core provider + core Message）重写为 kernel-native，落在 `jeikcode-coding`，然后把 3 个消费者（cli、daemon 两处）重指向它，使它们不再依赖 `core::provider` / `core::vision_preprocessor`。
...
消费者（3 处，都要脱离 core）：
- **cli** `crates/jeikcode-cli/src/vision.rs`：`VlImagePreprocessor` 实现 coding 的 `ImagePreprocessor` seam，内部 `create_provider` + `maybe_preprocess`（core）。outcome→UserInput 的映射（`apply_outcome`）**已是 kernel-native**，可复用。
...
- 新 `jeikcode-coding/src/vision.rs`：
  ...
  - `pub async fn run_vl_caption(vl_provider: Arc<dyn jeikcode_kernel::provider::LlmProvider>, vl_model: String, caption: &str, images: &[ImageContent]) -> PreprocessOutcome`（只返 Replaced|Failed）：构造 prompt（逐字保留 core 中文文案）+ kernel `Message::user_with_images(prompt, images.to_vec())` + **async** `vl_provider.chat_stream(&msgs, &[], &ChatOptions::default())`，消费 `StreamEvent::TextDelta` 累积、`Done`→break、`Error`/空→Failed；30s **空闲**超时（`tokio::time::timeout` 包 `stream.next()`）。
- `jeikcode-coding` 放置：coding 已依赖 kernel/capabilities/config/telemetry，有 `CodingProviderFactory`/`derive_tier_config`/`CodingRuntimeConfig::from_config`，cli+daemon 都依赖它。
...
- **单测（coding vision）**：`should_skip` 纯逻辑（空图→true；视觉模型名→true；非视觉模型+有图→false）。`run_vl_caption` 用一个**测试替身** kernel provider（实现 `jeikcode_kernel::provider::LlmProvider`，`chat_stream` 返脚本化 `BoxStream`：几个 `TextDelta`+`Done`→断言 `Replaced{text=拼接}`; 直接 `Error`→`Failed`; 空流→`Failed`）——真正测流式累积/终结/错误映射，无网络。
