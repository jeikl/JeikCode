...
> `jeikcode-bridge` 已退役。仍未完成的是北极星架构中的非引擎 core/foundation/protocol
> 拆分，不应从本清单推导迁移状态。
...
- **Zero `jeikcode-core` change** — `cargo tree -p <crate>` shows 0 jeikcode-core; `git status --short -- crates/jeikcode-core` empty after every commit.
