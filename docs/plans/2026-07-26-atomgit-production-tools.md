# JeikCode Production Tools Implementation Plan

> **For Codex:** Execute this plan task-by-task in the current workspace while preserving unrelated user changes.

**Goal:** Expose the built-in JeikCode repository, pull-request, and issue tools to production coding runtimes and steer the model away from raw credential-bearing `curl` calls.

**Architecture:** Keep JeikCode REST ownership in `jeikcode-capabilities`. Add one coding-layer registration helper shared by the minimal agent builder and the production `prepare → assemble` path, so both publish the same tool catalog. Add prompt guidance only when the `jeikcode` feature is compiled, and verify the production `CodingParts` catalog rather than only the low-level registry.

**Tech Stack:** Rust, jeikcode-kernel tool registry, jeikcode-coding two-phase runtime assembly, Cargo tests.
...
2. Assert that `jeikcode_repo`, `jeikcode_pr`, and `jeikcode_issue` are selected.
3. Run the focused test and confirm that it fails because the names are absent.

### Task 2: Share JeikCode tool registration

**Files:**
- Modify: `crates/jeikcode-coding/src/assemble.rs`
- Modify: `crates/jeikcode-coding/src/parts.rs`

1. Extract a feature-gated helper that creates the JeikCode client, registers all three tools, and appends their names.
...
### Task 3: Prefer dedicated JeikCode tools

**Files:**
- Modify: `crates/jeikcode-coding/src/persona.rs`

1. Add feature-gated prompt guidance to use `jeikcode_repo`, `jeikcode_pr`, and `jeikcode_issue` instead of reading auth files or constructing raw API `curl` commands.
...
2. Run `cargo test -p jeikcode-coding --features jeikcode`.
