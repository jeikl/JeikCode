#!/usr/bin/env bash
# JeikCode headless-mode smoke tests
#
...
#   JEIKCODE_TEST_PROVIDER   Provider name to use for live (network) tests
...
TMPDIR_T=$(mktemp -d /tmp/jeikcode-headless-XXXXXX)
...
# run_jeik <secs> <stdout_file> <stderr_file> <args...>
...
echo "=== JeikCode Headless Smoke Tests ==="
echo "  Binary  : $BIN"
echo "  Timeout : $TIMEOUT_BIN"
echo "  TmpDir  : $TMPDIR_T"
echo "  Provider: ${JEIKCODE_TEST_PROVIDER:-<unset — network tests will be skipped>}"
echo ""
...
run_jeik 5 "$out" "$err" --headless || rc=$?
...
run_jeik 5 "$out" "$err" --provider __nonexistent_qa_probe__ -p "x" || rc=$?
...
###############################################################################
if [ -z "${JEIKCODE_TEST_PROVIDER:-}" ]; then
    skip "T1: -p emits stdout                  (needs JEIKCODE_TEST_PROVIDER)"
    skip "T2: stdout has no decoration markers (needs JEIKCODE_TEST_PROVIDER)"
    skip "T3: -v stderr has log/diagnostic output (needs JEIKCODE_TEST_PROVIDER)"
    skip "T3b: default headless stderr is clean    (needs JEIKCODE_TEST_PROVIDER)"
    skip "T4: -p does not block on stdin       (needs JEIKCODE_TEST_PROVIDER)"
else
    PROV="$JEIKCODE_TEST_PROVIDER"
    ...
    ###########################################################################
    # T4: -p with stdin closed must not block. The </dev/null in run_jeik
