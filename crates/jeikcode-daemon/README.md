...
```json
{
  "path": "/home/user/.jeikcode/config.toml",
...
```json
{
  "logged_in": true,
  "expired": false,
  "auth_path": "/home/user/.jeikcode/auth.toml",
  "user": {
...
重新加载 MCP 配置（从 `~/.jeikcode/mcp.json`）。
...
daemon 使用以下配置文件（位于 `~/.jeikcode/` 目录）：
