...
pub(crate) fn runtime_start_is_session_in_use(error: &jeikcode_coding::RuntimeStartError) -> bool {
    matches!(
        error,
...
fn handle_from_embedded_control(session_id: &str) -> Option<jeikcode_coding::CodingRuntimeHandle> {
    let control = {
...
fn cache_hub_execution_handle(session_id: &str, handle: jeikcode_coding::CodingRuntimeHandle) {
    let dir = hub()
        .execution_working_dir()
        .unwrap_or_else(|| PathBuf::from("."));
...
pub fn existing_runner_handle(session_id: &str) -> Option<jeikcode_coding::CodingRuntimeHandle> {
    let reg = jeikcode_coding::session_runtime_registry::SessionRuntimeRegistry::global();
...
pub async fn ensure_registry_runner(
    ...
    let session_mode = if let Ok(snapshot) = load_snapshot(&working_dir, &session_id) {
        ...
        jeikcode_coding::SessionMode::Draft {
            id: session_id.clone(),
        }
    };
    ...
    let _ = handle
        .wait_mcp_ready(jeikcode_capabilities::mcp::FIRST_TURN_SOFT_WAIT)
        .await;
    ...
    tokio::spawn(async move {
        ...
        while let Some(envelope) = events.recv().await {
            if let jeikcode_coding::CodingRuntimeEvent::SessionNameSuggested { name } =
                &envelope.event
            {
                let bucket =
                    jeikcode_capabilities::session::SessionManager::project_hash(&forward_dir);
...
pub fn registry_session_snapshot(
    ...
    if let Some(snap) = jeikcode_coding::session_runtime_registry::SessionRuntimeRegistry::global()
...
pub async fn submit_via_registry(
...
) -> Result<jeikcode_coding::SubmitReceipt, String> {
    let handle = jeikcode_coding::session_runtime_registry::SessionRuntimeRegistry::global()
    ...
    let working_dir = jeikcode_coding::session_runtime_registry::SessionRuntimeRegistry::global()
        .lookup(&session_id.to_string())
        .map(|e| e.working_dir)
        .unwrap_or_else(|| PathBuf::from("."));
    let _ = jeikcode_coding::session_runtime_registry::SessionRuntimeRegistry::global()
        .ensure_and_push_view(
            session_id.to_string(),
            working_dir,
            jeikcode_coding::session_runtime_registry::SessionViewEvent::InputAccepted {
                input: echo_input,
                client_input_id,
            },
        );
...
fn dual_write_runtime_event_to_registry(
    generation: u64,
    event: jeikcode_coding::CodingRuntimeEvent,
...
pub fn publish(
    binding: &LiveBinding,
    event: jeikcode_coding::SequencedRuntimeEvent,
) -> Result<(), HubError> {
...
pub async fn submit_confirmed(
    input: UserInput,
) -> Result<jeikcode_coding::SubmitReceipt, HubError> {
...
pub async fn reload_provider(
    ...
    provider_fingerprint: String,
) -> Result<jeikcode_coding::RuntimeGeneration, HubError> {
    hub()
...
pub async fn resume_session(
    session_id: String,
) -> Result<jeikcode_coding::SessionChanged, HubError> {
    ...
    if binding.session_id == session_id {
        ...
        return Ok(jeikcode_coding::SessionChanged {
            generation: jeikcode_coding::RuntimeGeneration(binding.generation),
    ...
    let project_bucket =
        jeikcode_capabilities::session::SessionManager::project_hash(&binding.working_dir);
...
pub(crate) fn session_delete_release(
    ...
    execution_session_id: Option<&str>,
    execution_busy: bool,
    registry_activity: Option<jeikcode_coding::session_runtime_registry::RuntimeActivity>,
) -> SessionDeleteRelease {
    use jeikcode_coding::session_runtime_registry::RuntimeActivity;
...
pub async fn reload_capabilities() -> Result<jeikcode_coding::SessionChanged, HubError> {
...
pub async fn ensure_headless_runtime(
    ...
    let (session_mode, initial_snapshot) = match requested_session_id {
        ...
        None => (
            jeikcode_coding::SessionMode::Fresh,
    ...
    let binding = bind_after_mcp_ready(
        handle.wait_mcp_ready(jeikcode_capabilities::mcp::FIRST_TURN_SOFT_WAIT),
        || {
            hub()
                .bind_with_provider(
                    session_id.clone(),
    ...
    tokio::spawn(async move {
        while let Some(event) = events.recv().await {
            let session_change = match &event.event {
                jeikcode_coding::CodingRuntimeEvent::SessionChanged(changed) => {
                    Some((changed.session_id.clone(), changed.working_dir.clone()))
                }
                _ => None,
            };
            if let jeikcode_coding::CodingRuntimeEvent::SessionNameSuggested { name } = &event.event
            {
                let bucket = jeikcode_capabilities::session::SessionManager::project_hash(
...
mod tests {
    ...
    fn session_in_use_detects_prepare_wrapped_lease_error() {
        let error = jeikcode_coding::RuntimeStartError::Prepare(std::io::Error::other(
