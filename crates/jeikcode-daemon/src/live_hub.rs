...
use jeikcode_kernel::event::{AgentEvent, RequestId};
...
pub struct FreshSessionOutcome {
    pub changed: jeikcode_coding::SessionChanged,
...
fn correlate_web_steers_locked(
    state: &mut HubState,
    inputs: &[jeikcode_kernel::event::SteeredInput],
) -> Vec<Option<String>> {
    let steers = state.exec_steers_mut();
    inputs
        .iter()
...
impl LiveViewHub {
    ...
    pub fn execution_handle(&self) -> Result<jeikcode_coding::CodingRuntimeHandle, HubError> {
        let state = self.state.lock().unwrap_or_else(|error| error.into_inner());
        ...
        if announce {
            self.publish_view_locked(
                ...
                LiveViewEvent::Runtime(CodingRuntimeEvent::SessionChanged(
                    jeikcode_coding::SessionChanged {
                        generation: jeikcode_coding::RuntimeGeneration(identity.generation),
    ...
    pub async fn set_mode(&self, mode: jeikcode_coding::RuntimeMode) -> Result<(), HubError> {
        let (binding, handle) = self.bound_handle()?;
        handle
            .set_mode(mode)
            .await
    ...
    async fn resolve_pending_approvals_after_auto(
        ...
        let ids: Vec<jeikcode_kernel::event::RequestId> = {
            let mut state = self.state.lock().unwrap_or_else(|error| error.into_inner());
            state
                .exec_pending_mut()
                .iter()
                .filter(|(_, kind)| kind.as_str() == jeikcode_capabilities::tools::APPROVAL_KIND)
                .map(|(id, _)| *id)
                .collect()
        };
        if ids.is_empty() {
        ...
        let value = serde_json::to_value(jeikcode_capabilities::tools::ApprovalResponse::allow())
            .unwrap_or(serde_json::Value::Null);
        for id in ids {
            match handle.respond(id, value.clone()).await {
                Ok(()) | Err(jeikcode_coding::RuntimeError::StaleRequest { .. }) => {}
                Err(_) => continue,
            }
            let mut state = self.state.lock().unwrap_or_else(|error| error.into_inner());
    ...
    pub fn switch_view_only(
        ...
        if session_id == current_sid {
            return Ok(jeikcode_coding::SessionChanged {
                generation: jeikcode_coding::RuntimeGeneration(generation),
    ...
    pub async fn resume_session_with_lease(
        ...
        session_id: String,
        working_dir: PathBuf,
        lease: jeikcode_capabilities::session::SessionLease,
    ) -> Result<jeikcode_coding::SessionChanged, HubError> {
        let (binding, handle) = self.bound_handle()?;
        let changed = handle
    ...
    pub async fn reload_capabilities(&self) -> Result<jeikcode_coding::SessionChanged, HubError> {
        let (binding, handle) = self.bound_handle()?;
        let changed = handle
    ...
    pub fn publish(
        ...
        let apply_agent_config_reload = matches!(
            ...
            && jeikcode_capabilities::config_reload::take_pending_live_reload();
    ...
    fn commit_reconfigure_generation(
        &self,
        binding: &LiveBinding,
        generation: jeikcode_coding::RuntimeGeneration,
...
fn map_session_transition_error(error: jeikcode_coding::RuntimeError) -> HubError {
    match error {
        jeikcode_coding::RuntimeError::Busy => HubError::ActiveTurn,
...
fn session_view_from_live(
    event: &LiveViewEvent,
) -> Option<jeikcode_coding::session_runtime_registry::SessionViewEvent> {
...
mod tests {
    ...
    fn web_steer_correlation_uses_runtime_payload_and_returns_client_identity() {
        ...
        let folded = vec![jeikcode_kernel::event::SteeredInput {
    ...
    fn same_generation_and_identity_is_a_noop_session_change() {
        ...
        let unchanged = jeikcode_coding::SessionChanged {
            generation: jeikcode_coding::RuntimeGeneration(7),
        ...
        let changed = jeikcode_coding::SessionChanged {
            generation: jeikcode_coding::RuntimeGeneration(8),
            ..unchanged
        };
        ...
        hub.publish(
            ...
            SequencedRuntimeEvent {
                ...
                event: CodingRuntimeEvent::Reconfigured {
                    operation: jeikcode_coding::ReconfigureKind::Provider,
                },
            },
        )
        .unwrap();
    ...
    fn busy_session_transition_maps_to_active_turn() {
        assert_eq!(
            map_session_transition_error(jeikcode_coding::RuntimeError::Busy),
            HubError::ActiveTurn
        );
        assert_eq!(
            map_session_transition_error(jeikcode_coding::RuntimeError::Unavailable),
    ...
    fn driver_commands_and_local_inputs_share_the_bound_runtime() {
        ...
        hub.dispatch(DriverCommand::SetMode(jeikcode_coding::RuntimeMode::Plan))
            .unwrap();
        ...
        assert!(matches!(
            commands.lock().unwrap().as_slice(),
            [DriverCommand::SetMode(jeikcode_coding::RuntimeMode::Plan)]
        ));
        assert!(matches!(
    ...
    fn session_change_fails_join_closed_until_matching_snapshot_is_committed() {
        ...
        hub.publish(
            ...
            SequencedRuntimeEvent {
                generation: 1,
                sequence: 1,
                event: CodingRuntimeEvent::SessionChanged(jeikcode_coding::SessionChanged {
    ...
    fn confirmed_reconfigure_commits_generation_before_followup_commands() {
        ...
        *control.status.lock().unwrap() = RuntimeStatus {
        ...
        hub.commit_reconfigure_generation(&binding, jeikcode_coding::RuntimeGeneration(2), None)
            .unwrap();
        ...
        *control.status.lock().unwrap() = RuntimeStatus {
        ...
        assert_eq!(
            hub.commit_reconfigure_generation(
                &binding,
                jeikcode_coding::RuntimeGeneration(2),
