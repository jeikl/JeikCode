...
use jeikcode_capabilities::session::manager::SessionOrigin;
use jeikcode_capabilities::session::manager::{NativeImportCommitOutcome, META_VERSION};
...
pub struct ConvertedLegacySession {
    pub snapshot: jeikcode_kernel::message::SessionSnapshot,
...
impl From<jeikcode_capabilities::session::LoadedSession> for CatalogSessionView {
    fn from(value: jeikcode_capabilities::session::LoadedSession) -> Self {
        Self {
            snapshot: value.snapshot,
            meta: value.meta,
            presentation: value.presentation,
    ...
    let mut converted = match &message.content {
        ...
        LegacyContent::AssistantWithToolCalls {
            ...
            let mut converted = KernelMessage::assistant(
                ...
                tool_calls
                    .iter()
                    .map(|call| jeikcode_kernel::tool::ToolCall {
                        id: call.id.clone(),
            ...
            converted.reasoning_blocks = thinking_blocks
                .iter()
                .map(|block| jeikcode_kernel::message::ReasoningBlock {
...
fn convert_legacy_session_with_diagnostic(
    ...
    let mut snapshot = jeikcode_kernel::message::SessionSnapshot::new(snapshot_messages);
...
fn adopt_unconfirmed_native(
    ...
    mut expected_snapshot: Option<jeikcode_kernel::message::SessionSnapshot>,
...
) -> anyhow::Result<Vec<jeikcode_capabilities::session::CatalogEntry>> {
...
pub(crate) fn repair_catalog_names_for_display_in_root(
    sessions_root: &std::path::Path,
    entries: &mut [jeikcode_capabilities::session::CatalogEntry],
) {
...
fn working_dirs_equivalent(left: &std::path::Path, right: &std::path::Path) -> bool {
    jeikcode_capabilities::pathnorm::path_case_key(left)
        == jeikcode_capabilities::pathnorm::path_case_key(right)
}
...
fn load_catalog_session_view_in_root(
    ...
    use jeikcode_capabilities::session::CatalogPresence;
...
fn report_catalog_diagnostics(diagnostics: &[jeikcode_capabilities::session::CatalogDiagnostic]) {
...
fn reject_matching_catalog_diagnostic(
    diagnostics: &[jeikcode_capabilities::session::CatalogDiagnostic],
    query: &str,
) -> anyhow::Result<()> {
...
fn rebase_converted_turn_ids(
    ...
    for entry in &mut converted.presentation.entries {
        if let jeikcode_capabilities::session::DisplayAnchor::AfterTurn { turn_id } =
            &mut entry.anchor
        {
            *turn_id = turn_id
                .checked_add(base)
...
fn repair_metadata_only_sidecars(
    ...
    let presentation_has_imported_anchor = presentation.entries.iter().any(|entry| {
        matches!(
            entry.anchor,
            jeikcode_capabilities::session::DisplayAnchor::AfterTurn { turn_id }
...
mod tests {
    ...
    fn catalog_diagnostics_only_block_the_damaged_session() {
        use jeikcode_capabilities::session::{CatalogDiagnostic, CatalogDiagnosticKind};
    ...
    fn prepared_resume_keeps_exact_bucket_and_holds_cutover_lease() {
        ...
        for (bucket, text) in [(first_bucket, "first"), (second_bucket, "second")] {
            ...
            let snapshot =
                jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user(text)]);
        ...
        assert!(matches!(
            ...
            Err(jeikcode_capabilities::session::SessionStoreError::SessionInUse { .. })
        ));
    ...
    fn catalog_view_load_succeeds_while_another_runtime_holds_the_lease() {
        ...
        let lease = manager.acquire_lease(id).unwrap();
        let snapshot =
            jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user("hello")]);
    ...
    fn full_legacy_fixture_converts_to_expected_kernel_snapshot() {
        ...
        let snapshot = jeikcode_kernel::message::SessionSnapshot::new(msgs);

        assert_eq!(snapshot.version, jeikcode_kernel::message::SNAPSHOT_VERSION);
    ...
    fn legacy_only_cutover_commits_once_and_is_idempotent() {
        use jeikcode_capabilities::session::{SessionManager, StorageOwner};
    ...
    fn valid_native_snapshot_is_never_overwritten_by_legacy() {
        ...
        let session = full_legacy_session();
        let id = session.id.as_str();
        let native = jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user(
            "native wins",
        )]);
    ...
    fn empty_unconfirmed_native_stub_is_replaced_by_populated_legacy() {
        ...
        let empty_snapshot = jeikcode_kernel::message::SessionSnapshot::new(Vec::new());
    ...
    fn metadata_only_import_preserves_an_existing_native_presentation() {
        ...
        let legacy: LegacySession = serde_json::from_slice(legacy_bytes).unwrap();
        let native_snapshot =
            jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user(
                "native wins",
            )]);
    ...
    fn metadata_only_import_recomputes_after_full_state_cas_conflict() {
        ...
        let mut stale_snapshot =
            jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user("stale")]);
        stale_snapshot.turn_counter = 1;
        let mut fresh_snapshot =
            jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user("fresh")]);
    ...
    fn metadata_only_import_preserves_native_stats_after_legacy_zero_id_prefix() {
        ...
        let (converted, diagnostic) = convert_legacy_session_with_diagnostic(&legacy).unwrap();
        let mut snapshot =
            jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user("native")]);
    ...
    fn owner_native_missing_snapshot_never_falls_back_to_legacy() {
        ...
        let session = full_legacy_session();
        let id = session.id.as_str();
        let mut meta = jeikcode_capabilities::session::SessionMeta::new(id, "/p", 1);
        meta.owner = StorageOwner::Native;
        manager.write_meta(&meta).unwrap();
        std::fs::write(
            manager.legacy_path(id).unwrap(),
    ...
    fn complete_unconfirmed_native_without_legacy_is_adopted() {
        use jeikcode_capabilities::session::{SessionManager, SessionMeta, StorageOwner};
    ...
    fn project_scoped_rename_ignores_duplicate_id_in_other_bucket() {
        use jeikcode_capabilities::session::{PresentationFile, SessionManager, SessionMeta};
        ...
        for (bucket, name) in [(first_bucket, "first"), (second_bucket, "second")] {
            ...
            let lease = manager.acquire_lease(id).unwrap();
            let snapshot =
                jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user(name)]);
    ...
    fn catalog_session_view_repairs_complete_native_placeholder_name() {
        ...
        let manager = SessionManager::with_root(dir.path().join(bucket));
        let lease = manager.acquire_lease(id).unwrap();
        let snapshot = jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user(
            "恢复自动命名",
        )]);
        let presentation = PresentationFile::default();
        let mut meta = SessionMeta::new(id, "/project", 1);
    ...
    fn project_catalog_includes_equivalent_working_dir_from_historical_bucket() {
        ...
        for (bucket, id, stored_working_dir) in [
            ...
            let lease = manager.acquire_lease(id).unwrap();
            let snapshot =
                jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user(id)]);
    ...
    fn project_catalog_excludes_other_working_dir_from_historical_bucket() {
        ...
        let snapshot =
            jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user("other")]);
    ...
    fn project_catalog_name_repair_failure_does_not_hide_healthy_sessions() {
        ...
        let healthy_snapshot =
            jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user("健康会话")]);
        ...
        let damaged_snapshot =
            jeikcode_kernel::message::SessionSnapshot::new(vec![KernelMessage::user("损坏会话")]);
        manager
    ...
    fn native_ui_append_uses_stable_turn_anchor_and_not_runtime_snapshot() {
        use jeikcode_capabilities::session::{DisplayAnchor, TurnStat};
