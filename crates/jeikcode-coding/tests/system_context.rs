...
"#;
    ...
    handle
        .commands
        .send(jeikcode_kernel::event::AgentCommand::SendMessage {
            text: "first message".into(),
            images: vec![],
        })
        .unwrap();
    while let Some(ev) = handle.events.recv().await {
        if matches!(ev, jeikcode_kernel::event::AgentEvent::TurnComplete { .. }) {
            break;
        }
    }
...
"#;
    ...
    handle
        .commands
        .send(jeikcode_kernel::event::AgentCommand::Shutdown)
        .unwrap();
