...
use jeikcode_capabilities::reminder::{
...
use jeikcode_kernel::provider::{ChatOptions, ToolChoice};

use jeikcode_config::config::TodoEagerness;
...
impl LifecycleHooks for TodoEagerHook {
    ...
    async fn pre_request_options(
        ...
        if self.eagerness == TodoEagerness::Always && self.should_activate(messages, ctx) {
            options.tool_choice =
                ToolChoice::Specific(jeikcode_capabilities::tools::TODO_TOOL_NAME.to_string());
        }
    }
}
...
mod tests {
    ...
    async fn always_selects_todowrite_only_without_an_existing_list() {
        ...
        assert_eq!(
            options.tool_choice,
            ToolChoice::Specific(jeikcode_capabilities::tools::TODO_TOOL_NAME.into())
        );
