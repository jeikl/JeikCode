...
use jeikcode_kernel::agent::AgentHandle;
use jeikcode_kernel::checkpoint::CompactionCheckpointError;
use jeikcode_kernel::event::{AgentCommand, AgentEvent, RequestId, StopReason};
pub use jeikcode_kernel::message::CompactTrigger;
...
pub struct McpStatusSnapshot {
    ...
    pub servers: Vec<(String, jeikcode_capabilities::mcp::ServerStatus)>,
}
...
pub struct McpToolsSnapshot {
    pub generation: RuntimeGeneration,
    pub server: String,
    pub status: Option<jeikcode_capabilities::mcp::ServerStatus>,
...
fn ai_session_naming_enabled_for(runtime: &RuntimeResources) -> bool {
    runtime
        ...
        .subagent_config
        .as_deref()
        .map(jeikcode_config::config::ai_session_naming_enabled)
        .unwrap_or_else(|| {
            jeikcode_config::config::Config::load(&jeikcode_config::config::Config::default_path())
                .map(|config| jeikcode_config::config::ai_session_naming_enabled(&config))
                .unwrap_or(false)
        })
}
...
pub async fn compact_snapshot(
    ...
    let strategy = jeikcode_capabilities::compaction::OverflowCompaction::new(
...
fn snapshot_mutation(
    ...
    plan: &jeikcode_kernel::message::CompactionPlan,
                        ...
                        .await
                        {
                            ...
                            Ok(Err(error)) => {
                                let compensation_failed = matches!(
                                    &error,
                                    jeikcode_capabilities::session::WorkspaceCheckpointError::Compensation { .. }
                                );
...
fn resolve_reprepare_input(
    ...
    match target {
        ...
        ReprepareTarget::ChangeDirectory(directory) => {
            ...
            let canonical =
                jeikcode_capabilities::pathnorm::canonicalize(&target).map_err(|e| {
            ...
            let current =
                jeikcode_capabilities::pathnorm::canonicalize(&runtime.config.working_dir)
                    .unwrap_or_else(|_| runtime.config.working_dir.clone());
            if jeikcode_capabilities::pathnorm::path_case_key(&canonical)
                == jeikcode_capabilities::pathnorm::path_case_key(&current)
            {
                return Ok(None);
            }
            let mut config = runtime.config.clone();
...
mod tests {
    ...
    impl LlmProvider for MutatingProvider {
        ...
        async fn chat_stream(
            &self,
            _messages: &[Message],
            _tools: &[jeikcode_kernel::tool::ToolDef],
            _options: &jeikcode_kernel::provider::ChatOptions,
        ) -> Result<
    ...
    impl CodingProviderFactory for RecoverableAuthFactory {
        fn build(
            ...
            if self.fail.load(Ordering::Acquire) {
                ...
                Ok(Arc::new(jeikcode_kernel::testkit::MockProvider::new(
                    vec![],
                )))
            }
        }
    ...
    impl CodingProviderFactory for FailAfterFirstBuildFactory {
        fn build(
            ...
            if self.builds.fetch_add(1, Ordering::AcqRel) == 0 {
                Ok(Arc::new(jeikcode_kernel::testkit::MockProvider::new(vec![
                    vec![
                        jeikcode_kernel::stream::StreamEvent::TextDelta("answer".into()),
    ...
    impl CodingProviderFactory for BlockAndFailSecondBuildFactory {
        fn build(
            ...
            if self.builds.fetch_add(1, Ordering::AcqRel) == 0 {
                return Ok(Arc::new(jeikcode_kernel::testkit::MockProvider::new(vec![
                    vec![
                        jeikcode_kernel::stream::StreamEvent::TextDelta("answer".into()),
                        jeikcode_kernel::stream::StreamEvent::Done { truncated: false },
                    ],
    ...
    impl CodingProviderFactory for DeletePresentationAndFailSecondBuildFactory {
        fn build(
            ...
            if self.builds.fetch_add(1, Ordering::AcqRel) == 0 {
                return Ok(Arc::new(jeikcode_kernel::testkit::MockProvider::new(
                    vec![],
                )));
            }
    ...
    impl CodingProviderFactory for GoalNotMetProviderFactory {
        fn build(
            ...
            Ok(Arc::new(jeikcode_kernel::testkit::MockProvider::new(vec![
                vec![
                    jeikcode_kernel::stream::StreamEvent::TextDelta(
    ...
    impl CodingProviderFactory for GoalMetProviderFactory {
        fn build(
            ...
            Ok(Arc::new(jeikcode_kernel::testkit::MockProvider::new(vec![
                vec![
                    jeikcode_kernel::stream::StreamEvent::TextDelta("Verdict: yes goal met".into()),
                    jeikcode_kernel::stream::StreamEvent::Done { truncated: false },
                ],
            ])))
        }
    ...
    fn tier_provider(model: &str, rank: i64) -> jeikcode_config::config::provider::ProviderConfig {
    ...
    async fn runtime_start_installs_configured_subagent_tiers() {
        let mut routing = jeikcode_config::config::Config::default();
        routing
            .providers
    ...
    fn persist_native_session(
        manager: &jeikcode_capabilities::session::SessionManager,
        id: &str,
    ...
    async fn recoverable_failures_resume_with_context_even_when_compact_fails() {
        ...
        kernel_events
            .send(AgentEvent::CompactionFailed {
                trigger,
                error: jeikcode_kernel::checkpoint::CompactionCheckpointError::new(
                    "checkpoint failed",
                ),
            })
            .unwrap();
    ...
    impl CodingProviderFactory for PanicProviderFactory {
        fn build(
            ...
            use jeikcode_kernel::stream::ProviderError;
    ...
    async fn cancelled_first_turn_does_not_build_ai_naming_provider() {
        ...
        config.subagent_config = Some(Arc::new(jeikcode_config::config::Config::default()));
        let parts =
            prepare_with_plugin_hook_source(&config, prepare.clone(), plugin_hooks.as_ref())
                .await
                .unwrap();
    ...
    async fn native_undo_rollback_merges_concurrent_sidecar_updates() {
        ...
        manager
            ...
            .unwrap();
        let rollback_started_at = jeikcode_capabilities::session::now_ms();
