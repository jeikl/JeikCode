...
fn resolve_prompts_dir() -> Option<PathBuf> {
    ...
    // Unit tests must not leak the developer's `~/.jeikcode/prompts` into persona
    // snapshots. Production still loads the user home; tests opt in via JEIKCODE_HOME / ATOMCODE_HOME.
    #[cfg(test)]
    {
        return None;
    }
    #[cfg(not(test))]
    {
        ...
        let jeik = home.join(".jeikcode").join("prompts");
        if jeik.exists() {
            return Some(jeik);
        }
...
/// Bundled first-run templates for `~/.jeikcode/prompts/`. Existing files are never
...
fn detect_windows_platform() -> String {
    ...
    jeikcode_capabilities::process_utils::suppress_console_window_sync(&mut reg);
...
pub fn detect_git_branch(dir: &std::path::Path) -> Option<String> {
    ...
    if branch.is_empty() {
        ...
        head_cmd
            ...
            .current_dir(dir);
        jeikcode_capabilities::process_utils::suppress_console_window_sync(&mut head_cmd);
...
pub(crate) fn render_init_environment_from_with_git(
    ...
    let cwd = working_dir
        .map(|p| jeikcode_capabilities::pathnorm::to_display(p))
        .unwrap_or_else(|| {
            std::env::current_dir()
                .map(|p| jeikcode_capabilities::pathnorm::to_display(&p))
