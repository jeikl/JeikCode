...
use jeikcode_capabilities::tools::{bash_invocations, BashInvocation};
...
use jeikcode_kernel::tool::{Tool, ToolCall};
...
impl ToolMiddleware for TurnExecutionPolicy {
    async fn before(
        ...
        if !jeikcode_capabilities::tools::is_shell_tool_name(&call.name) {
