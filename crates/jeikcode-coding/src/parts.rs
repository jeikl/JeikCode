...
use jeikcode_capabilities::cc_hooks::{CCExternalHooks, HookConfig};
use jeikcode_capabilities::datalog::DatalogHook;
use jeikcode_capabilities::mcp::{self, McpRegistry};
use jeikcode_capabilities::memory::MemoryHook;

use jeikcode_capabilities::session::snapshot::SnapshotPersistenceStatus;
...
use jeikcode_capabilities::skills::{
...
use jeikcode_kernel::message::{Message, Role, SessionSnapshot};
...
struct McpWorkGuard {
    ...
    owns_registry: bool,
    _session_lease: Option<jeikcode_capabilities::mcp::SessionMcpLease>,
...
pub struct CodingParts {
    ...
    pub mcp_plan_grants: std::sync::Arc<dyn jeikcode_capabilities::tools::PermissionStore>,
    pub write_approval_grants: std::sync::Arc<dyn jeikcode_capabilities::tools::PermissionStore>,
    pub bash_workspace_grants: std::sync::Arc<dyn jeikcode_capabilities::tools::PermissionStore>,
    pub sensitive_path_grants: std::sync::Arc<dyn jeikcode_capabilities::tools::PermissionStore>,
...
/// plugins, which the DRIVER resolves through `jeikcode-capabilities::plugin` and
...
async fn prepare_with_plugin_hooks_reusing_lease(
    ...
    names.extend(
        jeikcode_capabilities::tools::coding_tool_names()
            .iter()
            .map(|s| s.to_string()),
    );
    ...
    if !todo_enabled {
        names.retain(|name| name != jeikcode_capabilities::tools::TODO_TOOL_NAME);
    }
    ...
    names.extend(
        jeikcode_capabilities::codeintel::codeintel_tool_names()
            .iter()
            .map(|s| s.to_string()),
    );
    ...
    names.extend(
        jeikcode_capabilities::skills::skill_tool_names()
            .iter()
            .map(|s| s.to_string()),
    );
    ...
    let (mcp_registry, owns_mcp_registry) = if !opts.mcp {
        (None, false)
    } else if let Some(shared) = opts.shared_mcp_registry.clone() {
        (Some(shared), false)
    } else {
        let registry = jeikcode_capabilities::mcp::ProjectMcpPool::global()
        ...
        if let Some(binding) = &session {
            Some(
                jeikcode_capabilities::mcp::SessionMcpPool::global()
                    .acquire(&cfg.working_dir, &binding.id)
                    .await,
            )
        } else {
            None
    ...
    let session_mcp_registry = session_mcp_lease
        .as_ref()
        .map(jeikcode_capabilities::mcp::SessionMcpLease::registry);
    ...
    hooks.push(Arc::new(jeikcode_capabilities::session::UserWrapHook::new(
        &cfg.working_dir,
    )));
    if opts.memory {
    ...
    let mcp_work_guard = McpWorkGuard {
        ...
        _session_lease: session_mcp_lease,
    ...
    Ok(CodingParts {
        ...
        mcp_catalog_ready: tokio::sync::watch::channel(
            mcp_registry.is_none() && session_mcp_registry.is_none(),
        )
        .0,
...
impl CodingParts {
    ...
    fn mount(&mut self) -> MountedTools {
        ...
        let mcp_registries: Vec<_> = self
            .mcp_registry
            .iter()
            .chain(self.session_mcp_registry.iter())
            .cloned()
            .collect();
        if !mcp_registries.is_empty() {
            ...
            tokio::spawn(async move {
                ...
                // session-scoped browser tool never disappears from turn one.
                let waits = mcp_registries.iter().map(|registry| async move {
                    if registry.initial_connections_complete() {
                        return true;
                    }
                    tokio::select! {
                        _ = registry.wait_until_initial_connections_done() => true,
                        _ = registry.wait_for_cancellation() => false,
                    }
                });
                if futures::future::join_all(waits)
                    .await
                    .into_iter()
                    .all(|ready| ready)
                {
    ...
    pub(crate) async fn mcp_statuses(
        ...
        if let Some(registry) = &self.mcp_registry {
            statuses.extend(registry.server_statuses().await);
        }
        if let Some(registry) = &self.session_mcp_registry {
            statuses.extend(registry.server_statuses().await);
        }
        statuses.sort_by(|left, right| left.0.cmp(&right.0));
        statuses.dedup_by(|left, right| left.0 == right.0);
        statuses
    }
    ...
    pub(crate) fn mcp_tools_for_server(&self, server: &str) -> Vec<String> {
        ...
        self.mcp_registry
            .iter()
            .chain(self.session_mcp_registry.iter())
            .flat_map(|registry| registry.tool_aliases_for_server(server))
    ...
    pub fn register_extra_tool(&mut self, tool: Arc<dyn jeikcode_kernel::tool::Tool>) {
        ...
        for info in tool_infos {
            match jeikcode_capabilities::mcp::McpToolAdapter::new(mcp_registry.clone(), info) {
                Ok(adapter) if aliases.insert(adapter.full_name().to_string()) => {
                    adapters.push(Arc::new(adapter) as Arc<dyn jeikcode_kernel::tool::Tool>);
                }
                Ok(adapter) => {
                    eprintln!(
                        "[mcp] cross-scope tool alias collision skipped: {}",
                        adapter.full_name()
                    );
                }
                Err(error) => eprintln!("[mcp] tool publication skipped: {error}"),
            }
        }
    }
...
pub fn assemble(
    ...
    let out_of_loop_provider: Arc<dyn LlmProvider> = match &parts.session {
        Some(session) => {
            let mut recorder = jeikcode_capabilities::session::DetachedUsageRecorder::new(
            ...
            Arc::new(jeikcode_capabilities::session::UsageRecordingProvider::new(
                provider.clone(),
                recorder,
            ))
        }
    ...
    if let Some(b) = &parts.session {
        ...
        // Share the parent's `x-jeikcode-session-id` / `x-session-id` with the subagent tier providers so a
                ...
                crate::subagent_tiers::resolve_tier_keys(registry, &cfg.model)
            {
                let install_recorder = |cell: &Arc<crate::config::TierProvider>, key: &str| {
                    ...
                    if let Ok(resolved) = registry.resolve_model(Some(key)) {
                        ...
                        let mut recorder =
                            jeikcode_capabilities::session::DetachedUsageRecorder::new(
                                b.manager.clone(),
                                &b.id,
                                key,
                                &resolved.model,
    ...
    if let Some(store) = artifact_store {
        let threshold = cfg
            ...
            .unwrap_or(jeikcode_capabilities::tools::THRESHOLD_BYTES);
...
fn check_snapshot_version(snap: &SessionSnapshot) -> io::Result<()> {
    if snap.version != jeikcode_kernel::message::SNAPSHOT_VERSION {
        return Err(io::Error::new(
            ...
            format!(
                ...
                snap.version,
                jeikcode_kernel::message::SNAPSHOT_VERSION
            ),
        ));
    }
    Ok(())
...
pub fn subagent_runtime_knobs(
    cfg: &jeikcode_config::config::SubAgentConfig,
...
mod tests {
    ...
    impl jeikcode_kernel::tool::Tool for TestMcpTool {
        fn name(&self) -> &str {
            "mcp__test__echo"
        }
    ...
    fn subagent_runtime_knobs_ignore_legacy_timeout_and_floor_concurrency() {
        ...
        use jeikcode_config::config::SubAgentConfig;
    ...
    fn reconcile_persona_embeds_config_working_dir_not_process_cwd() {
        ...
        assert_ne!(
            jeikcode_capabilities::pathnorm::to_display(&process_cwd),
            jeikcode_capabilities::pathnorm::to_display(&switched),
        ...
        let process_display = jeikcode_capabilities::pathnorm::to_display(&process_cwd);
        assert!(
    ...
    async fn mcp_tools_for_server_uses_exact_alias_ownership() {
        ...
        let info = jeikcode_capabilities::mcp::McpToolInfo {
        ...
        let adapter =
            jeikcode_capabilities::mcp::McpToolAdapter::new(registry.clone(), info).unwrap();
        let alias = jeikcode_kernel::tool::Tool::name(&adapter).to_string();
    ...
    fn session_store_error(
        error: &io::Error,
    ) -> &jeikcode_capabilities::session::SessionStoreError {
        error
            .get_ref()
            .and_then(|source| {
                source.downcast_ref::<jeikcode_capabilities::session::SessionStoreError>()
            })
        ...
        assert!(matches!(
            error
                ...
                .and_then(|source| source.downcast_ref::<
                    jeikcode_capabilities::session::SessionStoreError,
                >()),
            Some(jeikcode_capabilities::session::SessionStoreError::SessionInUse {
                id,
                ..
            }) if id == "same-session"
        ));
    ...
    impl LlmProvider for CannedProvider {
        ...
        async fn chat_stream(
            &self,
            _: &[Message],
            _: &[jeikcode_kernel::tool::ToolDef],
            _: &jeikcode_kernel::provider::ChatOptions,
        ) -> Result<
    ...
    async fn primary_telemetry_hook_tracks_model_swapped_at_assemble() {
        ...
        let chat = records
            .iter()
            .find(|r| matches!(r.event, jeikcode_telemetry::Event::LlmChat { .. }))
    ...
    async fn snapshot_cost_attribution_tracks_model_swapped_at_assemble() {
        ...
        let report = jeikcode_capabilities::session::aggregate_session_cost(
    ...
    async fn artifact_wiring_store_middleware_and_fetch_roundtrip() {
        ...
        use jeikcode_kernel::middleware::ToolMiddleware;
        use jeikcode_kernel::tool::Tool as _;
        ...
        let id = jeikcode_capabilities::tools::artifact_id(big.as_bytes());
