...
use jeikcode_kernel::middleware::{AfterOutcome, BeforeOutcome, ToolMiddleware};
...
impl Attribution {
    ...
    fn new(
        ...
        let provider_host = jeikcode_telemetry::resolve_provider_host(&vendor, Some(base_url));
        Self {
            telemetry,
...
///
/// [`OverflowCompaction`]: jeikcode_capabilities::compaction::OverflowCompaction
...
mod tests {
    ...
    async fn denied_tool_emits_tool_call_denied() {
        use jeikcode_kernel::testkit::EchoTool;
        let (tel, captured) = Telemetry::in_memory("test".into());
