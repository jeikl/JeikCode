...
use jeikcode_telemetry::queue::Queue;
use jeikcode_telemetry::sender::{http::HttpSender, SenderRuntime};
use std::sync::Arc;
...
async fn sender_posts_and_deletes_segment() {
    ...
    let counters = Arc::new(jeikcode_telemetry::Counters::default());
...
async fn only_one_sender_can_claim_a_ready_segment() {
    ...
    let counters1 = Arc::new(jeikcode_telemetry::Counters::default());
    let counters2 = Arc::new(jeikcode_telemetry::Counters::default());
...
use jeikcode_telemetry::config::{ResolvedConfig, TelemetryState};
