...
impl Telemetry {
    pub fn init(cfg: ResolvedConfig, app_version: String) -> Arc<Self> {
        ...
        let device_id = match load_or_create(&cfg.jeikcode_dir) {
            Ok(id) => id,
            Err(e) => {
        ...
        let qdir = cfg.jeikcode_dir.join("telemetry/queue");
        ...
        let health_path = cfg.jeikcode_dir.join("telemetry/health.json");
    ...
    fn enabled_telemetry(dir: &TempDir) -> Arc<Telemetry> {
        Telemetry::init(
            ResolvedConfig {
                ...
                jeikcode_dir: dir.path().to_path_buf(),
            },
            "test".into(),
        )
    }
