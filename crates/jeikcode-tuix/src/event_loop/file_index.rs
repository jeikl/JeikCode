// crates/jeikcode-tuix/src/event_loop/file_index.rs
//
...
const ALWAYS_INDEX_DIRS: &[&str] = &[".claude", ".jeikcode", ".agents"];
...
mod tests {
    ...
    fn directory_at_mention_replacement_keeps_token_active() {
        let replacement = format_at_mention_replacement("crates/jeikcode-bridge/");

        assert_eq!(replacement, "@crates/jeikcode-bridge/");
        assert_eq!(
            detect_at_mention(&replacement, replacement.len()),
            Some("crates/jeikcode-bridge/".to_string())
        );
    }
    ...
    fn mock_index() -> FileIndex {
        FileIndex::from_entries(
            ...
            vec![
                ...
                Entry {
                    rel_path: ".jeikcode/".into(),
                    is_dir: true,
                    depth: 1,
                },
                Entry {
                    rel_path: "crates/jeikcode-cli/".into(),
                    is_dir: true,
                    depth: 2,
                },
                Entry {
                    rel_path: "crates/jeikcode-tuix/".into(),
                    is_dir: true,
                    depth: 2,
                },
                Entry {
                    rel_path: "crates/jeikcode-tuix/Cargo.toml".into(),
                    is_dir: false,
                    depth: 3,
                },
                Entry {
    ...
    fn filter_empty_returns_only_direct_children() {
        ...
        assert!(
            !names.contains(&"crates/jeikcode-tuix/"),
            "depth-2 should be hidden under empty filter: {:?}",
            names
        );
        assert!(
            !names.contains(&"crates/jeikcode-tuix/Cargo.toml"),
    ...
    fn filter_substring_matches_across_levels() {
        ...
        assert!(names.contains(&"crates/jeikcode-tuix/"), "got: {:?}", names);
    }
    ...
    // Feature: the well-known agent-config dirs (`.claude`, `.jeikcode`,
