...
use anyhow::Result;
use jeikcode_capabilities::memory::MemoryStore;
...
fn attach_observer_runtime(
    ...
    let handle = jeikcode_daemon::native_live::existing_runner_handle(&session.id)?;
    let runtime_id = ctx.bg_manager.allocate_runtime_id();
        ...
        loop {
            match rx.recv().await {
                Ok(sequenced) => {
                    ...
                    let envelope = jeikcode_coding::SequencedRuntimeEvent {
...
fn adopt_session_as_view(
    ...
    if !ctx.runtime.is_shared_observer() {
        let _ = ctx
            .runtime
            .dispatch_when_ready(jeikcode_coding::DriverCommand::Shutdown);
    }
    let (runtime_id, endpoint, session) = attach_or_spawn_runtime(ctx, session);
    ctx.bg_manager.set_foreground_runtime(
        runtime_id,
...
mod bg_live_guard_tests {
    ...
    fn failed_live_unbind_preserves_the_local_guard_binding() {
        ...
        let result = detach_live_binding_with(&mut binding, |_| {
            Err(jeikcode_daemon::live_hub::HubError::ActiveTurn)
        });
    ...
    fn resumed_request_is_prioritized_ahead_of_the_shared_runtime_queue() {
        let mut transport_queue = std::collections::VecDeque::from([RuntimeEventPayload::Ui(
            crate::event_loop::ui_event::UiEvent::TurnComplete {
                ...
                snapshot: jeikcode_kernel::message::SessionSnapshot::new(Vec::new()),
            },
        )]);
        ...
        assert!(matches!(
            queued,
            RuntimeEventPayload::Native(jeikcode_coding::CodingRuntimeEvent::Request(request))
                if request.id == 42
        ));
        assert!(matches!(
    ...
    fn switching_foreground_discards_outgoing_session_replay_requests() {
        let outgoing_request = jeikcode_coding::RuntimeRequest {
            id: 41,
            kind: jeikcode_capabilities::tools::APPROVAL_KIND.into(),
            payload: serde_json::json!({}),
            snapshot: None,
        };
        let resumed_request = jeikcode_coding::RuntimeRequest {
            id: 42,
            kind: jeikcode_capabilities::tools::APPROVAL_KIND.into(),
            payload: serde_json::json!({}),
            snapshot: None,
        ...
        let mut replay_queue = std::collections::VecDeque::from([RuntimeEventPayload::Native(
            jeikcode_coding::CodingRuntimeEvent::Request(outgoing_request),
        )]);

        schedule_resumed_runtime_replay(
            &mut replay_queue,
            vec![RuntimeEventPayload::Native(
                jeikcode_coding::CodingRuntimeEvent::Request(resumed_request),
            )],
        );
...
pub fn perform_session_rename(
    ...
    let old_name = jeikcode_daemon::legacy_convert::rename_catalog_session_in_project(
        project_bucket,
        session_id.as_str(),
        &new_name,
    )
...
/// writing `.jeikcode.md` (so users see the new file appear under
...
pub(crate) fn attach_live_runtime(
    ...
    let provider_fingerprint =
        jeikcode_daemon::native_live::provider_fingerprint(&ctx.config, &provider_selection)?;
    let binding = jeikcode_daemon::native_live::register_embedded_runtime(
    ...
    jeikcode_daemon::live_set_mode(mode);
    ctx.live_binding = Some(binding);
    let mut remote_commands = jeikcode_daemon::native_live::register_remote_command_sink();
    ...
    if let Ok(live_join) = jeikcode_daemon::native_live::join() {
...
fn project_live_view_event(
    event: jeikcode_daemon::live_hub::LiveViewEvent,
) -> Option<super::ui_event::UiEvent> {
    match event {
        jeikcode_daemon::live_hub::LiveViewEvent::InputAccepted { input, .. } => {
        ...
        jeikcode_daemon::live_hub::LiveViewEvent::RequestResolved { request_id, kind } => {
...
fn detach_live_runtime(ctx: &mut LoopCtx) -> Result<bool, String> {
    ...
    detach_live_binding_with(&mut ctx.live_binding, |binding| {
        jeikcode_daemon::native_live::unregister_embedded_runtime(binding)
    })
}

fn detach_live_binding_with(
    binding: &mut Option<jeikcode_daemon::live_hub::LiveBinding>,
    unregister: impl FnOnce(
        &jeikcode_daemon::live_hub::LiveBinding,
    ) -> Result<(), jeikcode_daemon::live_hub::HubError>,
) -> Result<bool, String> {
...
fn submit_agent_text(ctx: &LoopCtx, text: String) -> bool {
    if ctx.live_binding.is_some() {
        jeikcode_daemon::native_live::submit(jeikcode_coding::UserInput::from(text)).is_ok()
    } else {
        ctx.runtime
            .dispatch(jeikcode_coding::DriverCommand::Submit(text.into()))
            .is_ok()
    }
}
    ...
    match payload {
        crate::event_loop::loop_ctrl::LoopPayload::Prompt(text) => {
            let submitted = if ctx.live_binding.is_some() {
                jeikcode_daemon::native_live::submit(text.into()).is_ok()
            } else {
                ctx.runtime
                    .dispatch(jeikcode_coding::DriverCommand::Submit(text.into()))
                    .is_ok()
...
pub(crate) fn stop_active_loop(state: &mut UiState, ctx: &mut LoopCtx) {
    if ctx.loop_ctrl.is_some() || state.loop_label.is_some() {
        ctx.runtime
            .dispatch(jeikcode_coding::DriverCommand::StopLoop)
            .ok();
        ctx.loop_ctrl = None;
...
pub(super) fn execute_slash_command(
    ...
    if !cap.captured.is_empty() {
        if let Err(error) = jeikcode_daemon::native_live::publish_command_output(format!(
            "/{}\n{}",
...
fn relay_client_cache_dir() -> PathBuf {
    jeikcode_config::config::Config::config_dir().join("bin")
}
...
fn execute_slash_command_impl(
    ...
    match cmd {
        ...
        "plan" => {
            ...
            ctx.runtime
                .dispatch(jeikcode_coding::DriverCommand::SetMode(
                    jeikcode_coding::RuntimeMode::Plan,
                ))
                .ok();
            jeikcode_daemon::live_set_mode(AgentMode::Plan);
            renderer.render(UiLine::CommandOutput(
        ...
        "build" => {
            ...
            ctx.runtime
                .dispatch(jeikcode_coding::DriverCommand::SetMode(
                    jeikcode_coding::RuntimeMode::Build,
                ))
                .ok();
            jeikcode_daemon::live_set_mode(AgentMode::Build);
            renderer.render(UiLine::CommandOutput(
        ...
        "auto" => {
            ...
            ctx.runtime
                .dispatch(jeikcode_coding::DriverCommand::SetMode(
                    jeikcode_coding::RuntimeMode::Auto,
                ))
                .ok();
            jeikcode_daemon::live_set_mode(AgentMode::Auto);
            renderer.render(UiLine::CommandOutput(
        ...
        "language" => {
            if arg.is_empty() {
                *active_modal = Some(Box::new(LanguagePicker::open()));
                ...
                match arg.parse::<jeikcode_config::locale::Locale>() {
                    Ok(locale) => {
        ...
        "schedule" => {
            let now = std::time::SystemTime::now()
                ...
                .unwrap_or(0);
            let tasks = jeikcode_config::schedule::list();
        ...
        "app" => {
            ...
            let msg = if a == "stop" {
                ...
                let killed = ctx
                    ...
                    .is_some();
                let server_stopped = jeikcode_daemon::stop_app_server();
                ...
                match relay_base.filter(|s| !s.is_empty()) {
                    ...
                    Some(relay) => {
                        ...
                        jeikcode_daemon::stop_app_server();
                        let app_user_id: Option<String> = None;
                        let started = tokio::task::block_in_place(|| {
                            tokio::runtime::Handle::current().block_on(
                                jeikcode_daemon::ensure_app_server(
                                    "127.0.0.1",
                                    jeikcode_daemon::APP_DEFAULT_PORT,
                                    app_user_id,
                                ),
                            )
                        });
                        match started {
                            ...
                            Ok((_h, port)) => {
                                ...
                                let token = match jeikcode_auth::oauth::get_stored_auth() {
                                    Some(auth) => format!(
                                        "{}.{}",
                                        auth.user.id,
        ...
        "upgrade" => {
            ...
            if arg_norm == "rollback" {
                ...
                tokio::spawn(async move {
                    ...
                    if let Err(e) = jeikcode_updater::run_upgrade(current, force, tx.clone()).await
                    {
                        let _ = tx.send(jeikcode_updater::UpgradeEvent::Failed(format!("{:#}", e)));
                    }
                });
            }
        }
                ...
                bg_runtime::BgCommand::Drop(slot) => {
                    ...
                    if matches!(dropped.state, bg_runtime::RuntimeState::Running) {
                        if let Some(endpoint) = dropped.endpoint.as_ref() {
                            endpoint
                                .native
                                .dispatch(jeikcode_coding::DriverCommand::Cancel)
                                .ok();
                        }
                    }
        ...
        "background" => {
            ...
            let submit_result =
                endpoint
                    .native
                    .dispatch_when_ready(jeikcode_coding::DriverCommand::Submit(
                        task.to_string().into(),
                    ));
            if let Err(error) =
        ...
        "init" => {
            ...
            // static .jeikcode.md generator.
            submit_agent_turn(ctx, state, jeikcode_coding::INIT_PROMPT.to_string());
        ...
        "mcp" => {
            ...
            match parse_mcp_subcommand(sub) {
                Some(McpSub::Login) => {
                    ...
                    let configs =
                        match jeikcode_capabilities::mcp::load_mcp_config(&ctx.working_dir) {
                            Ok(configs) => configs,
                            Err(e) => {
                                renderer.render(UiLine::Error(
                    ...
                    let is_github_server = matches!(
                        &config.config,
                        jeikcode_capabilities::mcp::McpTransportConfig::Http {
                            auth: Some(jeikcode_capabilities::mcp::McpHttpAuthConfig::OAuth(auth)),
                            ..
                        } if auth.provider.as_deref() == Some("github")
                    );
                    let result = tokio::task::block_in_place(|| {
                        jeikcode_capabilities::mcp::login_mcp_oauth(
                            &config,
                ...
                Some(McpSub::Logout) => {
                    ...
                    let token_store = jeikcode_capabilities::mcp::McpTokenStore::default();
                ...
                Some(McpSub::Trust) => {
                    match jeikcode_capabilities::mcp::trust::trust_project(&ctx.working_dir) {
                        Ok(()) => {
                            renderer.render(UiLine::CommandOutput(
                ...
                Some(McpSub::Untrust) => {
                    if !jeikcode_capabilities::mcp::trust::is_project_trusted(&ctx.working_dir) {
                        renderer.render(UiLine::CommandOutput(
                    ...
                    match jeikcode_capabilities::mcp::trust::untrust_project(&ctx.working_dir) {
                        Ok(true) => {
                            renderer.render(UiLine::CommandOutput(
                ...
                Some(McpSub::Reload) => {
                    ...
                    let partition = jeikcode_capabilities::mcp::trust::partition_by_trust(
                        configs.clone(),
                        &ctx.working_dir,
                    );
        ...
        "goal" => {
            ...
            match head {
                ...
                "clear" | "stop" | "off" | "reset" | "none" | "cancel" => {
                    ctx.runtime
                        .dispatch(jeikcode_coding::DriverCommand::StopGoal)
                        .ok();
                ...
                _ => {
                    ...
                    if ctx
                        .runtime
                        .dispatch(jeikcode_coding::DriverCommand::StartGoal(condition.clone()))
                        .is_err()
                    {
        ...
        "loop" => {
            ...
            match parse_loop_arg(arg) {
                ...
                LoopArg::SelfPaced { prompt } => {
                    ...
                    if ctx
                        .runtime
                        .dispatch(jeikcode_coding::DriverCommand::StartLoop(prompt.clone()))
                        .is_err()
                    {
                        renderer.render(UiLine::Error(t(Msg::CmdProviderUnavailable).into_owned()));
                        renderer.flush();
        ...
        other => {
            ...
            // .jeikcode/skills, etc.). Both expand to a prompt and dispatch
                    ...
                    if let Some(rendered) = expand_skill(ctx, other, arg) {
                        ...
                        let available_commands: Vec<&str> =
                            crate::commands::CommandRegistry::builtin()
                                ...
                                .collect();
                        ctx.telemetry.track(jeikcode_telemetry::Event::UseCommand {
                            type_: other.to_string(),
                            success: Some(false),
                            error_kind: Some(jeikcode_telemetry::UseCommandErrorKind::NotFound),
                            error_data: Some(
                                serde_json::json!({
                                    "command": other,
                                    "duration_ms": 0,
    ...
    match sub {
            ...
            match action {
                "add" => {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let ev =
                            match jeikcode_capabilities::plugin::marketplace::add_marketplace(&url)
                            {
                                Ok(info) => {
                                    jeikcode_capabilities::plugin::PluginJobEvent::MarketplaceAdded(
                                        info,
                                    )
                                }
                                Err(e) => jeikcode_capabilities::plugin::PluginJobEvent::Failed {
                ...
                "remove" => {
                    match jeikcode_capabilities::plugin::marketplace::remove_marketplace(arg) {
                        Ok(()) => {
                ...
                "update" => {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let ev = match jeikcode_capabilities::plugin::marketplace::update_marketplace(&name)
                        {
                            Ok(info) => {
                                jeikcode_capabilities::plugin::PluginJobEvent::MarketplaceUpdated(info)
                            }
                            Err(e) => jeikcode_capabilities::plugin::PluginJobEvent::Failed {
                ...
                "list" => match jeikcode_capabilities::plugin::marketplace::list_marketplaces() {
        ...
        "install" => {
            ...
            match parse_plugin_arg(rest) {
                Some(PluginArg::Qualified {
                    ...
                    tokio::task::spawn_blocking(move || {
                        let ev = match jeikcode_capabilities::plugin::installer::install(&plugin, &mp, scope) {
                            Ok(info) => jeikcode_capabilities::plugin::PluginJobEvent::PluginInstalled(info),
                            Err(e) => {
                                if let Some(_aie) = e.downcast_ref::<jeikcode_capabilities::plugin::installer::AlreadyInstalledError>() {
                                    jeikcode_capabilities::plugin::PluginJobEvent::PluginAlreadyInstalled {
                                        id: _aie.id.clone(),
                                    }
                                } else {
                                    jeikcode_capabilities::plugin::PluginJobEvent::Failed {
                                        op: "install".into(),
                                        msg: format!("{:#}", e),
                                    }
                                }
                ...
                Some(PluginArg::Bare { plugin }) => {
                    ...
                    match jeikcode_capabilities::plugin::installer::resolve_plugin_marketplace(
                        &plugin,
                    ) {
                        Ok(matches) if matches.len() == 1 => {
                            ...
                            tokio::task::spawn_blocking(move || {
                                let ev = match jeikcode_capabilities::plugin::installer::install(&resolved_plugin, &mp, scope) {
                                    Ok(info) => jeikcode_capabilities::plugin::PluginJobEvent::PluginInstalled(info),
                                    Err(e) => {
                                        if let Some(_aie) = e.downcast_ref::<jeikcode_capabilities::plugin::installer::AlreadyInstalledError>() {
                                            jeikcode_capabilities::plugin::PluginJobEvent::PluginAlreadyInstalled {
        ...
        "uninstall" => match parse_plugin_arg(parts.next().unwrap_or("").trim()) {
            Some(PluginArg::Qualified {
                ...
                match jeikcode_capabilities::plugin::installer::uninstall(
                    &plugin,
                    &mp,
            ...
            Some(PluginArg::Bare { plugin }) => {
                ...
                let installed =
                    jeikcode_capabilities::plugin::installer::list_installed().unwrap_or_default();
                ...
                match matches.len() {
                    ...
                    1 => {
                        ...
                        match jeikcode_capabilities::plugin::installer::uninstall(&plug, &mp, scope)
                        {
                            Ok(()) => {
                                super::reload_plugins(ctx);
                                ok(
        ...
        "list" => match jeikcode_capabilities::plugin::installer::list_installed() {
            Ok(items) if items.is_empty() => {
...
fn parse_scope_arg(s: &str) -> jeikcode_capabilities::plugin::InstallScope {
    // Accept both `--scope user` and bare `user`.
    let val = s.strip_prefix("--scope=").unwrap_or(s).trim();
    match val.to_lowercase().as_str() {
        "project" => jeikcode_capabilities::plugin::InstallScope::Project,
        "local" => jeikcode_capabilities::plugin::InstallScope::Local,
        _ => jeikcode_capabilities::plugin::InstallScope::User,
    }
}
...
pub(crate) fn build_cost_report_text(
    mut report: jeikcode_capabilities::session::SessionCostReport,
    config: &jeikcode_config::config::Config,
    current_provider: &str,
    current_model: &str,
    current_pricing: Option<jeikcode_capabilities::session::ModelPricing>,
) -> String {
    if !report
        .models
        .iter()
        .any(|item| item.provider_id == current_provider && item.model_id == current_model)
    {
        report
            .models
            .push(jeikcode_capabilities::session::ModelCostSummary {
...
fn build_session_cost_text(ctx: &LoopCtx, state: &UiState) -> String {
    ...
    let pricing = ctx
        ...
        .and_then(|config| jeikcode_coding::resolve_provider_pricing(&provider, &config));
    ...
    let mut report = match manager.read_meta(&ctx.current_session.id) {
        Ok(meta) => jeikcode_capabilities::session::aggregate_session_cost(&meta),
        Err(_) => {
            ...
            jeikcode_capabilities::session::SessionCostReport {
                models: Vec::new(),
                unattributed_tokens,
    ...
    let live_tokens = jeikcode_capabilities::session::TokenBreakdown {
        input: state
            .turn_prompt_tokens
...
) -> jeikcode_capabilities::session::SessionManager {
    project_bucket
        .filter(|bucket| bucket.len() == 16 && bucket.bytes().all(|byte| byte.is_ascii_hexdigit()))
        .map(|bucket| {
            jeikcode_capabilities::session::SessionManager::with_root(
                jeikcode_capabilities::session::SessionManager::sessions_root().join(bucket),
            )
        })
        .unwrap_or_else(|| jeikcode_capabilities::session::SessionManager::for_project(working_dir))
}
...
pub(crate) fn build_schedule_list_text(
    tasks: &[jeikcode_config::schedule::ScheduleTask],
    now: i64,
) -> String {
    if tasks.is_empty() {
    ...
    for t in tasks {
        let next = jeikcode_config::schedule::next_run(&t.schedule, now)
            .map(|ts| format!("{ts}"))
...
fn apply_cd_with_effect(
    ...
    let path = jeikcode_capabilities::pathnorm::strip_verbatim_path(&path);
    ...
    ctx.pending_session_transition = Some(crate::event_loop::PendingSessionTransition {
        operation: jeikcode_coding::ReconfigureKind::ChangeDirectory,
...
pub(crate) fn push_recent_dir(dirs: &mut Vec<PathBuf>, new: PathBuf) {
    ...
    let key = jeikcode_capabilities::pathnorm::path_case_key(&new);
    dirs.retain(|d| jeikcode_capabilities::pathnorm::path_case_key(d) != key);
    dirs.insert(0, new);
...
fn parse_recent_dirs(contents: &str) -> Vec<PathBuf> {
    ...
    contents
        ...
        .map(PathBuf::from)
        .map(|p| jeikcode_capabilities::pathnorm::strip_verbatim_path(&p))
        .filter(|p| seen.insert(jeikcode_capabilities::pathnorm::path_case_key(p)))
        .collect()
}

/// Read `~/.jeikcode/recent_dirs.txt`. Silently drops missing directories
/// so stale entries from a deleted project don't linger in the picker.
pub(crate) fn load_recent_dirs() -> Vec<PathBuf> {
    let path = jeikcode_config::config::Config::config_dir().join("recent_dirs.txt");
...
/// Persist `dirs` to `~/.jeikcode/recent_dirs.txt`. Best-effort — a write
...
pub(crate) fn load_cd_picker_dirs(current: &Path, recent: &[PathBuf]) -> Vec<PathBuf> {
    let scan = jeikcode_capabilities::session::SessionManager::scan_catalog(
        &jeikcode_capabilities::session::SessionManager::sessions_root(),
    );
...
where
    ...
    F: FnMut(&Path) -> bool,
{
    let current = jeikcode_capabilities::pathnorm::strip_verbatim_path(current);
    let mut catalog_seen = std::collections::HashSet::new();
    let catalog_dirs = catalog_dirs
        .into_iter()
        .map(|path| jeikcode_capabilities::pathnorm::strip_verbatim_path(&path))
        .filter(|path| catalog_seen.insert(jeikcode_capabilities::pathnorm::path_case_key(path)))
        .filter(|path| is_dir(path))
        .collect::<Vec<_>>();
    let current_key = jeikcode_capabilities::pathnorm::path_case_key(&current);
    let current_in_catalog = catalog_dirs
        .iter()
        .any(|path| jeikcode_capabilities::pathnorm::path_case_key(path) == current_key);
    let mut seen = std::collections::HashSet::new();
    ...
    for path in catalog_dirs {
        if seen.insert(jeikcode_capabilities::pathnorm::path_case_key(&path)) {
            dirs.push(path);
        }
    }
    for path in recent {
        let path = jeikcode_capabilities::pathnorm::strip_verbatim_path(path);
        let key = jeikcode_capabilities::pathnorm::path_case_key(&path);
...
fn render_save_markdown(messages: &[jeikcode_kernel::message::Message]) -> Option<String> {
...
mod status_assemble_tests {
    ...
    fn status_body_no_longer_shows_a_token_line() {
        let en = jeikcode_config::i18n::t_with(
            jeikcode_config::i18n::Locale::En,
            Msg::StatusBody {
                model: "m",
                dir: "/d",
                config: "/c",
        ...
        let zh = jeikcode_config::i18n::t_with(
            jeikcode_config::i18n::Locale::ZhCn,
            Msg::StatusBody {
                model: "m",
                dir: "/d",
                config: "/c",
...
fn todo_clear_messages(id: String) -> Vec<jeikcode_kernel::message::Message> {
    ...
    let mut call = Message::assistant(
        ...
        vec![ToolCall {
            id: id.clone(),
            name: jeikcode_capabilities::tools::TODO_TOOL_NAME.to_string(),
...
fn todo_add_messages(id: String, content: &str) -> Vec<jeikcode_kernel::message::Message> {
    use jeikcode_kernel::message::Message;
    use jeikcode_kernel::tool::ToolCall;
...
fn reseed_todo_conversation(
    ctx: &mut LoopCtx,
    state: &mut UiState,
    pair: Vec<jeikcode_kernel::message::Message>,
) {
    let mut snapshot = ctx.current_session.to_conversation_snapshot();
    snapshot.messages.extend(pair);
    ctx.runtime
        .dispatch(jeikcode_coding::DriverCommand::RestoreSnapshot(
            snapshot.clone(),
        ))
        .ok();
    ctx.current_session
...
pub(crate) fn format_todo_command(
    ...
    let calls: Vec<(&str, &str)> = messages
        ...
        .collect();
    let todos = jeikcode_capabilities::tools::todo::reduce_todos(calls);
    if todos.is_empty() {
    ...
    format!(
        ...
        jeikcode_capabilities::tools::todo::render_todos_text(&todos, unicode)
    )
}
...
mod tests {
    ...
    fn live_request_resolution_projects_to_correlated_tui_event() {
        let event =
            project_live_view_event(jeikcode_daemon::live_hub::LiveViewEvent::RequestResolved {
                request_id: 42,
                kind: jeikcode_capabilities::tools::request_user_input::REQUEST_USER_INPUT_KIND
                    .into(),
            })
    ...
    fn active_session_bucket_uses_runtime_directory_not_embedded_metadata() {
        ...
        assert_eq!(
            bucket,
            jeikcode_capabilities::session::SessionManager::project_hash(&runtime_dir)
        );
        assert_ne!(
            bucket,
            jeikcode_capabilities::session::SessionManager::project_hash(&stale_meta_dir)
        );
    }
    ...
    fn make_dirs() -> (tempfile::TempDir, PathBuf, PathBuf) {
        let tmp = tempfile::tempdir().expect("tempdir");
        let strip = jeikcode_capabilities::pathnorm::strip_verbatim_path;
    ...
    fn tilde_expands_to_home() {
        ...
        let canon_home = jeikcode_capabilities::pathnorm::strip_verbatim_path(&canon_home);
        let (_tmp, cwd, _sub) = make_dirs();
...
mod todo_command_tests {
    ...
    fn init_submits_the_coding_init_prompt() {
        // handler 用 jeikcode_coding::INIT_PROMPT 作为提交文本;这里锁定接线源。
        assert!(jeikcode_coding::INIT_PROMPT.contains("AGENTS.md"));
    }

    #[test]
    fn cost_report_keeps_models_separate_and_hides_unknown_price() {
        use crate::event_loop::commands::build_cost_report_text;
        use jeikcode_capabilities::session::{ModelCostSummary, SessionCostReport, TokenBreakdown};
