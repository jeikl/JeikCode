...
pub const MAX_BACKGROUND_SLOTS: usize = jeikcode_coding::MAX_LIVE_SESSIONS;

/// Soft cap on concurrent live session runners (OpenCode: not "background slots").
pub const MAX_LIVE_SESSIONS: usize = jeikcode_coding::MAX_LIVE_SESSIONS;
...
pub enum RuntimeEventPayload {
    ...
    SequencedNative(jeikcode_coding::SequencedRuntimeEvent),
    Driver(DriverEvent),
}
...
pub enum DriverEvent {
    ...
    SessionTransitionFinished {
        operation: jeikcode_coding::ReconfigureKind,
        result: Result<jeikcode_coding::SessionChanged, jeikcode_coding::RuntimeError>,
    },
    SessionResumePrepared {
        project_bucket: String,
        session_id: String,
        working_dir: PathBuf,
        result: Result<jeikcode_daemon::legacy_convert::CatalogSessionView, String>,
    },
...
pub struct BackgroundSlot {
    ...
    pending_session_change: Option<jeikcode_coding::SessionChanged>,
    ...
    pending_request: Option<jeikcode_coding::RuntimeRequest>,
}
    ...
    pub fn apply_event_to_slot(&mut self, slot: usize, event: &AgentEvent) -> bool {
        ...
        match event {
            ...
            AgentEvent::WorkingDirChanged(working_dir) => {
                bg.working_dir = jeikcode_capabilities::pathnorm::strip_verbatim_path(working_dir);
                false
            }
...
fn load_background_session_projection(
    changed: &jeikcode_coding::SessionChanged,
...
fn load_background_session_projection_in_root(
    ...
    let working_dir = jeikcode_capabilities::pathnorm::strip_verbatim_path(&changed.working_dir);
    let project_bucket = jeikcode_capabilities::session::SessionManager::project_hash(&working_dir);
    let manager = jeikcode_capabilities::session::SessionManager::with_root(
...
fn resolve_background_session_projection(
    changed: &jeikcode_coding::SessionChanged,
    load: impl FnOnce(&jeikcode_coding::SessionChanged) -> Result<Session, String>,
    ...
    Ok((
        session,
        jeikcode_capabilities::pathnorm::strip_verbatim_path(&changed.working_dir),
    ))
}
...
impl BgRuntimeManager {
    ...
    pub fn reload_capabilities_for_working_dir(
        ...
        let working_dir = jeikcode_capabilities::pathnorm::strip_verbatim_path(working_dir);
    ...
    pub fn live_runner_bindings(
        ...
        bool,
        u64,
        Option<jeikcode_coding::CodingRuntimeHandle>,
    )> {
        self.backgrounds
            .slots
            .iter()
    ...
    fn apply_background_session_name_with(
        ...
        if !jeikcode_coding::session_title::should_accept_ai_name(
    ...
    fn apply_background_session_name(&mut self, runtime_id: RuntimeId, name: String) {
        self.apply_background_session_name_with(
            ...
            |working_dir, session_id, name| {
                ...
                jeikcode_daemon::legacy_convert::apply_ai_catalog_name_in_project(
                    &project_bucket,
                    session_id,
                    name,
                )
        ...
        let terminal = match event {
            ...
            RuntimeEventPayload::Native(CodingRuntimeEvent::WorkingDirectoryChanged(
                ...
                if let Some(bg) = self.backgrounds.slot_mut_for_runtime_id(runtime_id) {
                    bg.working_dir =
                        jeikcode_capabilities::pathnorm::strip_verbatim_path(&working_dir);
                }
                false
            }
            ...
            RuntimeEventPayload::Native(CodingRuntimeEvent::Agent(event)) => {
                if let Some(bg) = self.backgrounds.slot_mut_for_runtime_id(runtime_id) {
                    if !matches!(
                        &event,
                        jeikcode_kernel::event::AgentEvent::Cancelled
                            | jeikcode_kernel::event::AgentEvent::Request { .. }
                            | jeikcode_kernel::event::AgentEvent::Snapshot { .. }
                            | jeikcode_kernel::event::AgentEvent::TurnComplete { .. }
                    ) {
...
fn background_state_for_stop_reason(reason: jeikcode_kernel::event::StopReason) -> RuntimeState {
    match reason {
        jeikcode_kernel::event::StopReason::Stopped => RuntimeState::Done,
        jeikcode_kernel::event::StopReason::Cancelled => RuntimeState::Cancelled,
        _ => RuntimeState::Error,
    }
}
...
fn test_endpoint() -> RuntimeEndpoint {
    let (native, _controls) = jeikcode_coding::runtime::coding_runtime_control_channel();
    RuntimeEndpoint {
        native: native.into(),
    }
}
...
mod tests {
    ...
    fn render_empty_bg_list_mentions_no_background_sessions() {
        ...
        crate::i18n::set_locale(jeikcode_config::locale::Locale::En);
    ...
    fn a_new_background_turn_drops_the_previous_prompt_suggestion() {
        let mut events = vec![
            ...
            RuntimeEventPayload::Native(CodingRuntimeEvent::NextPromptSuggested {
                generation: jeikcode_coding::RuntimeGeneration(1),
    ...
    fn sequenced_native_turn_terminal_updates_background_snapshot_and_state() {
        ...
        use jeikcode_coding::{RuntimeTurnStats, SequencedRuntimeEvent, TurnCompletion};
        use jeikcode_kernel::{
            event::StopReason,
    ...
    fn sequenced_native_request_is_retained_for_foreground_resume() {
        ...
        use jeikcode_coding::{RuntimeRequest, SequencedRuntimeEvent};
        use jeikcode_kernel::message::{Message, SessionSnapshot};
    ...
    fn background_approval_does_not_change_selected_session_and_replays_on_return() {
        use jeikcode_coding::RuntimeRequest;
    ...
    fn sequenced_native_agent_output_is_retained_for_foreground_resume() {
        use jeikcode_coding::SequencedRuntimeEvent;
        ...
        manager.apply_background_event(
            ...
            RuntimeEventPayload::SequencedNative(SequencedRuntimeEvent {
                generation: 1,
                sequence: 1,
                event: CodingRuntimeEvent::Agent(jeikcode_kernel::event::AgentEvent::TextDelta(
                    "partial answer".into(),
                )),
            }),
        );
        ...
        assert!(matches!(
            ...
            [RuntimeEventPayload::Native(CodingRuntimeEvent::Agent(
                jeikcode_kernel::event::AgentEvent::TextDelta(text)
    ...
    fn background_request_replays_after_prior_agent_output() {
        ...
        manager.apply_background_event(
            RuntimeId::new(2),
            RuntimeEventPayload::Native(CodingRuntimeEvent::Agent(
                jeikcode_kernel::event::AgentEvent::TextDelta("before approval".into()),
            )),
        );
        manager.apply_background_event(
            RuntimeId::new(2),
            RuntimeEventPayload::Native(CodingRuntimeEvent::Request(
                jeikcode_coding::RuntimeRequest {
                    id: 7,
                    kind: jeikcode_capabilities::tools::APPROVAL_KIND.into(),
                    payload: serde_json::json!({}),
                    snapshot: None,
    ...
    fn request_snapshot_supersedes_already_buffered_turn_output() {
        ...
        manager.apply_background_event(
            RuntimeId::new(2),
            RuntimeEventPayload::Native(CodingRuntimeEvent::Agent(
                jeikcode_kernel::event::AgentEvent::TextDelta("streamed answer".into()),
            )),
        );
        manager.apply_background_event(
            RuntimeId::new(2),
    ...
    fn abnormal_native_turn_terminals_do_not_look_successful() {
        ...
        use jeikcode_coding::{RuntimeTurnStats, TurnCompletion};
        use jeikcode_kernel::{event::StopReason, message::SessionSnapshot};
    ...
    fn snapshot_failure_keeps_partial_output_available_for_resume() {
        use jeikcode_coding::{RuntimeSnapshotError, RuntimeTurnStats, TurnCompletion};
        ...
        manager.apply_background_event(
            RuntimeId::new(2),
            RuntimeEventPayload::Native(CodingRuntimeEvent::Agent(
                jeikcode_kernel::event::AgentEvent::TextDelta("partial answer".into()),
            )),
        );
        manager.apply_background_event(
            RuntimeId::new(2),
        ...
        assert!(matches!(
            resumed.replay_events.last(),
            Some(RuntimeEventPayload::Native(CodingRuntimeEvent::Agent(
                jeikcode_kernel::event::AgentEvent::Error { message, .. }
    ...
    async fn resume_restores_the_native_handle_for_that_runtime() {
        use jeikcode_coding::runtime::{coding_runtime_control_channel, CodingRuntimeControl};
    ...
    fn background_session_changed_replaces_the_complete_session_projection() {
        ...
        let changed = jeikcode_coding::SessionChanged {
            generation: jeikcode_coding::RuntimeGeneration(2),
    ...
    fn manual_compaction_finish_releases_background_runtime() {
        use jeikcode_coding::runtime::CompactionOutcome;
        use jeikcode_kernel::message::CompactTrigger;
        ...
        manager.apply_background_event(
            ...
            RuntimeEventPayload::Native(CodingRuntimeEvent::CompactionFinished {
                completion: CompactionCompletion::Interrupted {
                    trigger: CompactTrigger::Manual { focus: None },
                    reason: jeikcode_coding::runtime::CompactionInterruption::RuntimeReconfigured,
                },
            }),
        );
        ...
        manager.apply_background_event(
            ...
            RuntimeEventPayload::Native(CodingRuntimeEvent::CompactionFinished {
                completion: CompactionCompletion::Failed {
                    trigger: CompactTrigger::Manual { focus: None },
                    error: jeikcode_kernel::checkpoint::CompactionCheckpointError::new("disk full"),
                },
            }),
        );
