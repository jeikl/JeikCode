// crates/jeikcode-tuix/src/input/history.rs
...
/// `~/.jeikcode/image-cache/<hash>.<ext>`. Recorded on submit; consumed
...
impl History {
    ...
    /// Default history path: `~/.jeikcode/history` on Unix,
    /// `%USERPROFILE%\.jeikcode\history` on Windows (or a tempdir
