// crates/jeikcode-tuix/src/platform.rs
//
    ...
    if let Some(home) = home {
        ...
        if !home_str.is_empty() {
            ...
            if let Some(rest) = rest {
                ...
                // docs) which all reference `~/.jeikcode/...`.
...
pub fn image_cache_dir() -> PathBuf {
    jeikcode_config::config::Config::config_dir().join("image-cache")
}
...
mod tests {
    ...
    fn image_cache_dir_lives_under_config_dir() {
        ...
        let cfg = jeikcode_config::config::Config::config_dir();
