...
/// transport shares it) — see [`jeikcode_kernel::message::cold_summaries_from_messages`].
pub use jeikcode_kernel::message::cold_summaries_from_messages;
...
impl From<jeikcode_capabilities::session::TurnStat> for TurnStat {
    fn from(stat: jeikcode_capabilities::session::TurnStat) -> Self {
        Self {
...
impl From<jeikcode_capabilities::session::CatalogEntry> for SessionMeta {
    fn from(entry: jeikcode_capabilities::session::CatalogEntry) -> Self {
        Self {
            id: entry.id,
            name: entry.name,
...
impl TuiSession {
    ...
    pub fn from_catalog_view(
        view: jeikcode_daemon::legacy_convert::CatalogSessionView,
    ) -> anyhow::Result<Self> {
        ...
        let messages: Vec<Message> = view
            ...
            .filter(|m| {
                ...
                !(m.internal_origin.as_deref() == Some(LEGACY_COLD_SUMMARY_ORIGIN)
                    ...
                    && !jeikcode_capabilities::reminder::is_system_reminder(&m.text)
            })
            .cloned()
            .collect();
    ...
    pub fn touch(&mut self) {
        self.updated_at = jeikcode_capabilities::session::now_ms();
    }
...
mod tests {
    ...
    fn snapshot_round_trip_splits_and_reprepends_cold_summaries() {
        ...
        use jeikcode_kernel::message::{LEGACY_COLD_SUMMARY_ORIGIN, LEGACY_COLD_SUMMARY_PREFIX};
    ...
    fn catalog_display_messages_skip_injected_system_reminders() {
        ...
        let view = jeikcode_daemon::legacy_convert::CatalogSessionView {
            snapshot: SessionSnapshot::new(vec![Message::user("修复登录错误")]),
            meta: jeikcode_capabilities::session::SessionMeta::new(
                "reminder-session",
                "/project",
                1,
            ),
            presentation: PresentationFile {
                ...
                entries: vec![
                    PresentationEntry {
                        ...
                        text: jeikcode_capabilities::reminder::system_reminder(
                            "我就在任务1上！继续任务2！",
                        ),
                    },
                    PresentationEntry {
    ...
    fn catalog_display_messages_strip_reminder_tails_from_user_input() {
        ...
        let view = jeikcode_daemon::legacy_convert::CatalogSessionView {
            snapshot: SessionSnapshot::new(vec![Message::user(format!(
                "帮我看下端口\n\n{}",
                jeikcode_capabilities::reminder::system_reminder("Current date: 2026-09-01 (Tue)")
            ))]),
            meta: jeikcode_capabilities::session::SessionMeta::new(
            ...
            presentation: PresentationFile {
                ...
                entries: vec![PresentationEntry {
                    ...
                    text: format!(
                        "帮我看下端口\n\n{}",
                        jeikcode_capabilities::reminder::system_reminder(
