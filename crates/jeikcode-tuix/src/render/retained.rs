...
fn goal_row_parts_phase(
    ...
    max_cols: usize,
    unicode: bool,
    phase: jeikcode_coding::GoalPhase,
) -> (&'static str, String, String) {
    match phase {
        jeikcode_coding::GoalPhase::Pursuing => {
            goal_row_parts(condition, round, elapsed_secs, max_cols, unicode)
        }
        jeikcode_coding::GoalPhase::Paused => {
...
impl<W: Write + Send> RetainedRenderer<W> {
    ...
    fn render_inflight_tool(&mut self, icon: &str, name: &str, detail: &str, meta: &str) -> bool {
        ...
        let is_bash = jeikcode_capabilities::tools::is_shell_tool_name(&safe_name);
        if is_bash {
            crate::trace::pulse(
                "BASH",
                "inflight_paint",
    ...
    fn commit_inflight_tool(&mut self) {
        if let Some((_id, name, detail)) = self.inflight_tool.take() {
            ...
            if jeikcode_capabilities::tools::is_shell_tool_name(&safe_name)
        ...
        match line {
            ...
            UiLine::ToolResult {
                ...
                if collapsed.overflow {
                    let mut row = Vec::new();
                    let expand_icon = if self.caps.unicode_symbols {
                        "  ▼ "
                    } else {
                        "  v "
                    };
...
";
    ...
    fn subtask_panel_renders_fixed_compact_rows_and_hides_todo_panel() {
        ...
        status.todo = Some(crate::render::TodoProgress {
            ...
            total: 1,
            items: vec![(
                jeikcode_capabilities::tools::todo::TodoStatus::InProgress,
                "standing todo".into(),
            )],
        });
    ...
    fn subtask_panel_respects_short_screen_footer_budget() {
        ...
        status.goal = Some(GoalStatus {
            ...
            round: 2,
            elapsed_secs: 12,
            phase: jeikcode_coding::GoalPhase::Pursuing,
        });
        status.subtasks = Some(SubtaskProgress {
            call_id: "call-task".into(),
            completed: 0,
    ...
    fn retained_bash_inflight_without_hint_keeps_blank_above_running() {
        ...
        r.render(UiLine::ToolCallInFlight {
            id: "call-1".into(),
            name: "Bash".into(),
            detail: "cargo test -p jeikcode-tuix".into(),
            hint: None,
        });
        r.flush_deferred();
        drain_into_vterm(&buf, &mut vterm);
    ...
    fn retained_bash_live_tail_is_part_of_strip_and_cleared_on_commit() {
        ...
        assert_eq!(r.inflight_tool_rows, 0, "strip must be frozen after commit");
    }
    ...
    fn retained_footer_grow_then_push_catches_up_no_footer_overlap() {
        ...
        let whoami_text =
            "  TheoCui (saulcy)\n  cuizk@csdn.net\n  auth: /Users/theo/.jeikcode/auth.toml\n";
