...
use jeikcode_updater::{manifest_url, Manifest};
...
/// helpers in `jeikcode-capabilities`/`jeikcode-telemetry`.
fn apply_async_proxy_policy(builder: reqwest::ClientBuilder) -> reqwest::ClientBuilder {
    ...
    if jeikcode_config::tls::env_forces_tls12() {
...
mod tests {
    ...
    fn newer_major_version_returns_some() {
        assert_eq!(
            parse_and_compare("v4.15.3", &manifest_body("v5.0.5")),
            Some("v5.0.5".to_string())
        );
    }
