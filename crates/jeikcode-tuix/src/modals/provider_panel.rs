// crates/jeikcode-tuix/src/modals/provider_panel.rs
//
...
impl ProviderPanel {
    ...
    pub fn open_add_model(&mut self, config: &jeikcode_config::Config, wake_tx: WakeSender<()>) {
        self.tab = Tab::Models;
    ...
    fn save_model(&self, form: &ModelForm, ctx: &mut LoopCtx, renderer: &mut dyn Renderer) -> bool {
        ...
        let vl_configured = ctx
            .config
            .vision_preprocessor_provider
            .as_deref()
            .is_some_and(|k| !k.is_empty() && ctx.config.resolve_model(Some(k)).is_ok());
        ...
        update_config_and_reload(
            ...
            move |persisted| {
                ...
                let context_window = requested_window.unwrap_or_else(|| {
                    jeikcode_config::config::provider::default_context_window_for(wire)
                });
                ...
                let selection_id = if let Some(id) = &edit_id {
                    ...
                    let gemini =
                        jeikcode_capabilities::provider::model_supports_thinking(&model_name);
                    ...
                    // Make the selection-id key unique so a slash in the model
                    // name (or a repeat add) never silently overwrites a
                ...
                // Deferred provider api_key entered here fills the account once
                // — all its models share it.
            ...
            move |selection_id| {
                ...
                .into_owned();
                if supports_vision && vl_configured {
                    notice.push(' ');
                    notice.push_str(&crate::i18n::t(
                        crate::i18n::Msg::ProviderPanelImageDirectWhileVlSet,
                    ));
                }
                notice
            },
