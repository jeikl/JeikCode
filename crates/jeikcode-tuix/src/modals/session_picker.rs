...
impl Modal for SessionPicker {
            ...
            code if crate::input::key_action::is_ctrl_letter(code, mods, 'd') => {
                ...
                if self.confirm_delete == Some(idx) {
                    ...
                    if let Some(session) = self.sessions.get(idx) {
                        ...
                        match jeikcode_daemon::legacy_convert::delete_catalog_session_in_project(
            ...
            KeyCode::Enter => {
                ...
                let expected_bucket =
                    ctx.current_session_project_bucket
                        .clone()
                        .unwrap_or_else(|| {
                            jeikcode_capabilities::session::SessionManager::project_hash(
                                &ctx.working_dir,
                            )
                        });
                ...
                tokio::spawn(async move {
                    let result = tokio::task::spawn_blocking(move || {
                        jeikcode_daemon::legacy_convert::load_catalog_session_view_in_project(
    ...
    fn draw(&self, _buf: &Buffer, state: &UiState, ctx: &LoopCtx, renderer: &mut dyn Renderer) {
        ...
        let current_project_bucket =
            jeikcode_capabilities::session::SessionManager::project_hash(&ctx.working_dir);
...
fn humanize_age(ts_ms: i64) -> String {
    ...
    let d = jeikcode_capabilities::session::now_ms()
...
fn last_assistant_reply_markdown(messages: &[jeikcode_kernel::message::Message]) -> String {
...
fn is_real_user_message(message: &jeikcode_kernel::message::Message) -> bool {
    use jeikcode_kernel::message::Role;
...
pub(crate) fn replay_session(
    ...
    for (i, m) in session.messages.iter().enumerate() {
        ...
        match m.role {
            Role::User if is_real_user_message(m) => {
                let shown = jeikcode_capabilities::session::user_text_for_display(
                    &session.working_dir,
                    &m.text,
                );
                if !shown.is_empty() {
            ...
            Role::Assistant => {
                if m.internal_origin.as_deref()
                    == Some(jeikcode_kernel::message::TURN_DIAGNOSTIC_ORIGIN)
                {
                    if !m.text.is_empty() {
                    ...
                    if jeikcode_kernel::agent::distinct_tool_call_count(&m.tool_calls) > 1 {
                        ...
                        for tc in &m.tool_calls {
                            ...
                            // error still shows.
                            if jeikcode_capabilities::tools::is_todo_tool_name(&tc.name)
                                && jeikcode_capabilities::tools::todo::parse_todos(&tc.arguments)
                                    .is_ok()
                            {
                                if !tc.id.is_empty() {
...
mod tests {
    ...
    fn selecting_current_session_replays_the_ui_without_a_runtime_transition() {
        ...
        let current_bucket =
            jeikcode_capabilities::session::SessionManager::project_hash(&current.working_dir);
    ...
    fn replay_renders_reasoning_tools_and_turn_diagnostic() {
        use jeikcode_kernel::message::{Message, TURN_DIAGNOSTIC_ORIGIN};
        use jeikcode_kernel::tool::ToolCall;
