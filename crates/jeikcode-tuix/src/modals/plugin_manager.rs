// crates/jeikcode-tuix/src/modals/plugin_manager.rs
//
...
impl PluginManager {
    ...
    fn all_browse_plugins(&self) -> Vec<BrowsePluginItem> {
        ...
        let root_opt = jeikcode_capabilities::plugin::marketplaces_root();
        ...
        for m in &self.marketplaces {
            ...
            if let Some(ref root) = root_opt {
                ...
                if let Ok(Some(manifest)) =
                    jeikcode_capabilities::plugin::load_marketplace_manifest(&mp_dir)
                {
    ...
    fn reload(&mut self) {
        self.marketplaces =
            jeikcode_capabilities::plugin::marketplace::list_marketplaces().unwrap_or_default();
        self.installed =
    ...
    fn get_plugin_details(&self, plugin: &str, mp: &str) -> (Option<String>, Option<String>) {
        ...
        if let Some(root) = jeikcode_capabilities::plugin::marketplaces_root() {
                ...
                jeikcode_capabilities::plugin::load_marketplace_manifest(&mp_dir)
            {
                    ...
                    .find(|p| p.name == plugin || sanitize(&p.name) == plugin)
                {
                    ...
                    if let jeikcode_capabilities::plugin::PluginSource::Inline(ref path) = p.source
                    {
                        ...
                        if let Ok(manifest) =
                            jeikcode_capabilities::plugin::load_plugin_manifest(&dir)
                        {
            ...
            if version.is_none() && description.is_none() {
                ...
                if let Ok(manifest) =
                    jeikcode_capabilities::plugin::load_plugin_manifest(&plugin_dir)
                {
                ...
                if version.is_none() && description.is_none() {
                    if let Ok(manifest) =
                        jeikcode_capabilities::plugin::load_plugin_manifest(&mp_dir)
                    {
                        version = manifest.version;
                        description = manifest.description;
                    }
    ...
    fn dispatch_install(
        ...
        tokio::task::spawn_blocking(move || {
            let _ =
                jeikcode_capabilities::plugin::installer::uninstall(&plugin, &mp, scope.clone());
            let ev = match jeikcode_capabilities::plugin::installer::install(&plugin, &mp, scope) {
                ...
                Err(e) => {
                    if let Some(aie) = e
                        .downcast_ref::<jeikcode_capabilities::plugin::installer::AlreadyInstalledError>(
                    ) {
    ...
    fn dispatch_add(&mut self, url: String, ctx: &LoopCtx) {
        ...
        tokio::task::spawn_blocking(move || {
            let ev = match jeikcode_capabilities::plugin::marketplace::add_marketplace(&url) {
    ...
    fn dispatch_remove_marketplace(
        ...
        for i in installed_from_mp {
            if let Err(e) = jeikcode_capabilities::plugin::installer::uninstall(
                &i.plugin,
                &i.marketplace,
                i.scope,
            ) {
        ...
        match jeikcode_capabilities::plugin::marketplace::remove_marketplace(&name) {
            Ok(()) => {
                reload_plugins(ctx);
    ...
    fn enter_installed_details(&mut self, ctx: &mut LoopCtx, renderer: &mut dyn Renderer) {
        ...
        match self.selected {
            0 => {
                // Uninstall
                match jeikcode_capabilities::plugin::installer::uninstall(&plugin, &mp, scope) {
                    Ok(()) => {
                        reload_plugins(ctx);
                        renderer.render(UiLine::CommandOutput(
    ...
    fn rows(&self) -> (Vec<(String, String)>, String) {
        match &self.screen {
            ...
            Screen::Installed => {
                ...
                let root_opt = jeikcode_capabilities::plugin::plugins_root();
                let cwd = std::env::current_dir().ok();
                let mp_root_opt = jeikcode_capabilities::plugin::marketplaces_root();
                let mut descriptions = std::collections::HashMap::new();
                for i in &inst {
                    ...
                    let dir_opt = match i.scope {
                        ...
                        InstallScope::Project | InstallScope::Local => {
                            if let Some(ref wd) = cwd {
                                jeikcode_capabilities::plugin::project_plugins_root(wd, &i.scope)
                            } else {
                                None
                            }
                        }
    ...
    fn on_plugin_event(&mut self, ev: &PluginJobEvent) {
        ...
        if let PluginJobEvent::PluginInstalled(info) | PluginJobEvent::PluginUpdated(info) = ev {
            ...
            if self.cancelled_installs.take(&id).is_some() {
                ...
                let _ = jeikcode_capabilities::plugin::installer::uninstall(
                    &info.plugin,
                    &info.marketplace,
                    info.scope.clone(),
                );
...
fn get_directory_modified_date(name: &str) -> String {
    if let Some(root) = jeikcode_capabilities::plugin::marketplaces_root() {
        ...
        let target = if dir.join(".jeikcode-plugin/marketplace.json").exists() {
            dir.join(".jeikcode-plugin/marketplace.json")
