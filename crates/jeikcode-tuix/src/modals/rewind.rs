...
pub struct RewindModal {
    catalog: jeikcode_coding::RewindCatalog,
    stage: Stage,
...
impl RewindModal {
    pub fn open(catalog: jeikcode_coding::RewindCatalog) -> Self {
    ...
    fn selected_point(&self) -> Option<&jeikcode_capabilities::session::RewindPoint> {
        self.catalog.points.get(self.selected_target)
    }

    fn scope(&self) -> jeikcode_coding::RewindScope {
        match self.selected_scope {
            1 => jeikcode_coding::RewindScope::Code,
            2 => jeikcode_coding::RewindScope::ConversationAndCode,
            _ => jeikcode_coding::RewindScope::Conversation,
        }
    }
...
mod tests {
    ...
    fn catalog() -> jeikcode_coding::RewindCatalog {
        jeikcode_coding::RewindCatalog {
            generation: jeikcode_coding::RuntimeGeneration(1),
            revision: 1,
            points: vec![jeikcode_capabilities::session::RewindPoint {
                turn_id: 1,
                prompt_number: 1,
    ...
    fn bounded_target_window_keeps_default_current_row_visible() {
        ...
        catalog.points = (1..=20)
            .map(|turn_id| jeikcode_capabilities::session::RewindPoint {
                turn_id,
