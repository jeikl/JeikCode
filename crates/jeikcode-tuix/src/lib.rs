...
/// (`jeikcode-cli::restore_terminal_if_tui`) is then the only code with
...
/// by `jeikcode-cli`'s panic hook under `panic = "abort"`, where no Drop
...
pub async fn run(
    ...
    config_store: jeikcode_config::ConfigStore,
    ...
    let theme_light = match config.ui.theme {
        jeikcode_config::config::UiTheme::Light => true,
        jeikcode_config::config::UiTheme::Dark => false,
        jeikcode_config::config::UiTheme::Auto => {
    ...
    if let Ok(Some(pending)) = jeikcode_updater::read_pending() {
    ...
    let (upgrade_tx, upgrade_rx) =
        tokio::sync::mpsc::unbounded_channel::<jeikcode_updater::UpgradeEvent>();
    ...
    let (plugin_job_tx, plugin_job_rx) =
        tokio::sync::mpsc::unbounded_channel::<jeikcode_capabilities::plugin::PluginJobEvent>();
    ...
    let skill_registry = std::sync::Arc::new(std::sync::RwLock::new(
        jeikcode_capabilities::skills::SkillRegistry::new(),
    ));
    ...
    // burning 5–10s on every first launch.
    {
        ...
        tokio::spawn(async move {
            let events = tokio::task::spawn_blocking(move || {
                let events = jeikcode_capabilities::plugin::bootstrap::run_startup_hooks(&cfg);
    ...
    #[cfg(unix)]
    let _askpass_guard: Option<jeikcode_capabilities::askpass::server::AskpassServerGuard>;
    #[cfg(unix)]
    let askpass_rx: Option<
        tokio::sync::mpsc::Receiver<jeikcode_capabilities::askpass::server::AskpassPrompt>,
    >;

    #[cfg(unix)]
    {
        let cache = std::sync::Arc::new(jeikcode_capabilities::askpass::cache::PasswordCache::new(
            std::time::Duration::from_secs(300),
        ));
        match jeikcode_capabilities::askpass::server::start(cache) {
            Ok((mut env, rx, guard)) => {
                ...
                let script_result = std::env::current_exe().ok().and_then(|exe| {
                    env.sock_path.parent().and_then(|dir| {
                        jeikcode_capabilities::askpass::wrapper::write_askpass_script(&exe, dir)
                            .ok()
                    })
                });
                if let Some(script) = script_result {
                    env.askpass_script = script;
                    jeikcode_capabilities::askpass::set_env(env);
                }
                askpass_rx = Some(rx);
    ...
    // `.jeikcode.rolling` path after the swap, so we MUST use this saved
    // value instead.
...
mod panic_restore_tests {
    ...
    fn automatic_history_replay_cap_is_below_retained_memory_limit() {
        let cap =
            resolve_history_replay_max_rows(&jeikcode_config::Config::default(), &test_caps())
    ...
    fn configured_history_replay_cap_preserves_zero_and_custom_values() {
        let mut config = jeikcode_config::Config::default();
