...
- **想了解 `~/.jeikcode` 某个未知目录/文件的作用**：优先查阅 `topic: "directories"`。
...
| CLI `jeikcode mcp add` / `add-github-oauth` / `login` / `logout` | 写入 `.mcp.json` 或 `~/.jeikcode/mcp.json`，然后仍须 `/mcp reload` 或 `jeikcode_config_reload` |
