...
全局配置文件路径：`~/.jeikcode/config.toml`（或 `$JEIKCODE_HOME/config.toml`）。
...
### 3.2 自定义底层协议类型
- `openai-compatible` / `openai`：标准 OpenAI Chat Completions（`POST /chat/completions`）。`base_url` 写成含 `/v1` 的前缀，例如 `http://127.0.0.1:8046/v1`。
- `responses-compatible` / `responses` / `openai-responses`：OpenAI Responses 新协议（`POST /v1/responses`）。`base_url` 同样写成含 `/v1` 的前缀。
- `anthropic-compatible` / `anthropic` / `claude`：Anthropic Messages 协议。`base_url` 按自定义前缀拼接：未带版本时自动补 `/v1/messages`；已带 `/v1`、`/v2`、`/v3`、`/v1beta` 等版本段时接到对应的 `/vx/messages`；若已写到 `/messages` 则原样使用。
- `gemini-compatible` / `gemini` / `google-gemini`：Google Gemini 原生 `generateContent` 协议（`POST {base}/models/{model}:streamGenerateContent?alt=sse`）。默认 Base：`https://generativelanguage.googleapis.com/v1beta`。思考：`thinking_enabled=false` / `reasoning_effort=off` 关闭；2.5 用 `thinkingBudget`（low=1024 / medium=4096 / high=8192），**Gemini 3 及以上**用 `thinkingLevel`（LOW/MEDIUM/HIGH）；思考正文进 Reasoning 通道，`thoughtSignature` 多轮必须回传（`reasoning_history = "include"`，这是默认）。未设置时 Gemini 2.5 与 **3+**（含 3.1 / 3.8 / 4…）默认开思考。用量：`promptTokenCount` → prompt，`candidatesTokenCount + thoughtsTokenCount` → completion，`cachedContentTokenCount` → cached。
...
| **`reasoning_history`** | 字符串 | 推理模型 | **思考过程是否在多轮历史中回传**：<br>• `"exclude"`（推荐）：多轮交互时不把上一轮的 `<think>` 发回后端，大幅节省 Token 且防止某些模型报错。<br>• `"include"`：保留并回传思考历史（部分特殊 API 或中转网关/Gemini/DeepSeek 开启思考时要求；OpenAI 兼容与 Anthropic 协议均已打通支持）。 |
