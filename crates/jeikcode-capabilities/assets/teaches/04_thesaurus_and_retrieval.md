...
1. **工作区项目专属词林**：`<workspace>/.jeikcode/thesaurus/*.txt`（优先加载，适合业务专有领域术语）。
2. **全局用户词林**：`~/.jeikcode/thesaurus/*.txt`（全局生效）。
...
`~/.jeikcode/thesaurus/` 内置 9 大核心领域词林文件：
...
例如在当前项目根目录创建 `.jeikcode/thesaurus/biz_terms.txt`：
