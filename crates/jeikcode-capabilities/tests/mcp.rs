...
fn test_server_config_with_env_and_timeout(
    ...
    McpServerConfig {
        ...
        scope: jeikcode_capabilities::mcp::config::McpScope::Project,
    }
}
...
fn write_trusted_store(store_path: &std::path::Path, project_dir: &std::path::Path) {
    let key = jeikcode_capabilities::mcp::registry::project_trust_key(project_dir);
...
async fn session_scope_spawns_once_per_session_while_project_scope_stays_shared() {
    ...
    jeikcode_capabilities::mcp::refresh_session_mcp_schema(project.path()).await;
    assert_eq!(
        first.registry().connected_server_names().await,
        vec!["browser"],
