...
/// seeded as `~/.jeikcode/mcp.json` on first run (never overwrites existing).
...
/// vendored deps), seeded as `~/.jeikcode/.codegraphignore` on first run.
...
impl Tool for CodeExploreTool {
    ...
    async fn execute(&self, args: &str, ctx: &ToolContext) -> ToolResult {
        ...
        let path_str = match a.path.as_deref().map(str::trim) {
            ...
            _ => {
                return err(
                    "code_explore requires a `path` parameter naming the workspace root or a directory/module (e.g. '.', 'src/tools', 'crates/jeikcode-coding', 'backend').".to_string()
                );
            }
        };
...
mod tests {
    ...
    async fn identical_query_rewrites_frozen_incremental_headers() {
        use jeikcode_kernel::tool::{ProgressSink, Tool, ToolContext};
