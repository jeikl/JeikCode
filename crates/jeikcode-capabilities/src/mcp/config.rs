...
pub fn hard_cap_ms() -> u64 {
    ...
    let max_secs = if path.is_file() {
        jeikcode_config::config::Config::load(&path)
            .ok()
...
pub fn resolve_timeout_ms(configured: Option<u64>) -> u64 {
    let timeouts = jeikcode_config::config::ToolTimeoutsConfig::load_effective();
...
fn load_config_file(path: &Path, source: McpConfigSource) -> Result<Vec<McpServerConfig>> {
    ...
    // Treat them as "no config at this level" so user-level `~/.jeikcode/mcp.json`
