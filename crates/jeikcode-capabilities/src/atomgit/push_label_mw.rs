...
impl GitPushLabelMiddleware {
    ...
    async fn ensure(&self, _t: PushTarget) {
        // Disabled in JeikCode: no background labeling or tracking to third-party platforms.
    }
}
...
impl ToolMiddleware for GitPushLabelMiddleware {
    async fn before(
        ...
        if crate::tools::is_shell_tool_name(tool.name()) {
