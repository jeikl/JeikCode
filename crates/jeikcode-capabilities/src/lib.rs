//! jeikcode-capabilities — **L1 capabilities** layered on the neutral
//! `jeikcode-kernel` (L0).
//!
...
//!   - `provider` (default): real [`LlmProvider`](jeikcode_kernel::provider::LlmProvider)
...
/// Reusable, provider-agnostic [`jeikcode_kernel::hook::LifecycleHooks`]
...
/// [`jeikcode_kernel::message::CompactionStrategy`] that stubs old tool results in place.
...
/// Real, NEUTRAL coding [`Tool`](jeikcode_kernel::tool::Tool)s — fs `read`/`write`/
