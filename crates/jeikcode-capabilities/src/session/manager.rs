...
use jeikcode_kernel::message::{Conversation, Message, Role, SessionSnapshot, SNAPSHOT_VERSION};
...
impl SessionManager {
    ...
    pub fn project_hash(working_dir: &Path) -> String {
        jeikcode_config::util::stable_project_hash(working_dir)
    }
    ...
    pub fn load_native_session_for_resume(
        ...
        let replay_user = inflight.replay_safe
            ...
            && inflight.snapshot.messages.last().is_some_and(|message| {
                message.role == jeikcode_kernel::message::Role::User
                    && !message.synthetic
...
mod tests {
    ...
    fn rejects_future_snapshot_schema_in_the_store_reader() {
        ...
        snapshot.version = jeikcode_kernel::message::SNAPSHOT_VERSION + 1;
        std::fs::write(
    ...
    fn resume_recovers_inflight_assistant_state() {
        ...
        let inflight = SessionSnapshot::new(vec![
            ...
            Message::assistant(
                "waiting on approval",
                vec![jeikcode_kernel::tool::ToolCall {
                    id: "c1".into(),
