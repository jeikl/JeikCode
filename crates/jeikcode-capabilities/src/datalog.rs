...
use jeikcode_config::config::{Config, DatalogConfig};
...
impl DatalogHook {
    ...
    pub fn resolve_log_dir(working_dir: &Path, configured_dir: Option<&str>) -> PathBuf {
        let root = match configured_dir.filter(|value| !value.trim().is_empty()) {
            ...
            Some(value) if value.starts_with("~/") => jeikcode_config::util::real_home_dir()
...
impl ToolMiddleware for DatalogHook {
    async fn before(
        ...
        jeikcode_kernel::middleware::BeforeOutcome::Proceed
    }
...
fn project_slug(working_dir: &Path) -> String {
    ...
    let sanitized: String = basename
        ...
        .collect();
    let hash = jeikcode_config::util::stable_project_hash(working_dir);
