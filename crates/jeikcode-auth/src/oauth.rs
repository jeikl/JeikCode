...
use jeikcode_telemetry::{Event, Telemetry};
...
fn apply_blocking_proxy_policy(
    ...
    let builder = if std::env::var(jeikcode_config::proxy::MODE_ENV)
        .ok()
        .as_deref()
...
fn blocking_client() -> Result<reqwest::blocking::Client> {
    blocking_client_with_tls12(jeikcode_config::tls::should_cap_url(platform_base_url()))
}
...
fn pending_invite_for_login() -> (Option<String>, Option<uuid::Uuid>) {
    match jeikcode_telemetry::pending_invite::load(&Config::config_dir()) {
...
pub fn start_login() -> Result<LoginSession> {
    std::thread::spawn(move || {
        ...
        let was_capped = jeikcode_config::tls::should_cap_url(&login_url);
        match attempt_login(was_capped) {
            Err(first)
                if jeikcode_config::tls::should_try_fallback(
                    &login_url,
                    was_capped,
                ...
                ) =>
            {
                match attempt_login(true) {
                    Ok(session) => {
                        jeikcode_config::tls::latch_managed_tls12();
                        Ok(session)
                    }
...
pub fn auth_file_path() -> std::path::PathBuf {
    jeikcode_config::config::Config::config_dir().join("auth.toml")
}
