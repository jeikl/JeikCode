fn main() {
  ...
  <compatibility xmlns="urn:schemas-microsoft-com:compatibility.v1">
    ...
    // Auto-sync developer's ~/.jeikcode assets into crate asset directories at build time.
...
fn sync_user_home_assets_if_present() {
    ...
    // 1. Prompts: ~/.jeikcode/prompts/ -> crates/jeikcode-coding/assets/prompts/
    let user_prompts = home.join("prompts");
    if user_prompts.is_dir() {
        let dest = workspace_root.join("crates/jeikcode-coding/assets/prompts");
    ...
    // 2. Thesaurus: ~/.jeikcode/thesaurus/ -> crates/jeikcode-capabilities/assets/thesaurus/
    let user_thesaurus = home.join("thesaurus");
    if user_thesaurus.is_dir() {
        let dest = workspace_root.join("crates/jeikcode-capabilities/assets/thesaurus");
        let _ = std::fs::create_dir_all(&dest);
    ...
    // 3. Teaches: ~/.jeikcode/teaches/ -> crates/jeikcode-capabilities/assets/teaches/
    let user_teaches = home.join("teaches");
    if user_teaches.is_dir() {
        let dest = workspace_root.join("crates/jeikcode-capabilities/assets/teaches");
        let _ = std::fs::create_dir_all(&dest);
    ...
    for (src_rel, dest_rel) in [
        (
            "builtin-tools.txt",
            "crates/jeikcode-capabilities/assets/builtin-tools.txt",
        ),
        (
            ".codegraphignore",
            "crates/jeikcode-capabilities/assets/.codegraphignore",
        ),
        (
            "config_teachs.md",
            "crates/jeikcode-cli/assets/config_teachs.md",
        ),
        (
            "user-wrap.md",
            "crates/jeikcode-capabilities/assets/user-wrap.md",
        ),
    ] {
...
fn default_atomcode_home() -> std::path::PathBuf {
    #[cfg(windows)]
    {
        if let Ok(userprofile) = std::env::var("USERPROFILE") {
            return std::path::PathBuf::from(userprofile).join(".jeikcode");
        }
    }
    #[cfg(not(windows))]
    {
        if let Ok(home) = std::env::var("HOME") {
            return std::path::PathBuf::from(home).join(".jeikcode");
        }
    }
