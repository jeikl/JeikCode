...
use jeikcode_config::schedule::{self, Schedule, ScheduleTask};
...
/// corresponding [`jeikcode_coding::RuntimeMode`].  Unknown strings default to
/// `Plan` (safe default: read-only, never auto-approves destructive operations).
pub(crate) fn mode_from_str(s: &str) -> jeikcode_coding::RuntimeMode {
    match s {
...
async fn run_task(id: &str) -> Result<i32> {
    ...
    use jeikcode_coding::ProviderBootstrap;
    ...
    let effective_mode = if mode == jeikcode_coding::RuntimeMode::Auto {
    ...
    let notifications_cfg = if task.notify == "off" {
        jeikcode_config::config::NotificationConfig {
            enabled: false,
            ..Default::default()
        }
    } else {
...
mod tests {
    ...
    fn add_registers_via_os_scheduler() {
        with_temp_home(|| {
            ...
            jeikcode_config::schedule::save(&task).unwrap();
    ...
    fn remove_unregisters_via_os_scheduler() {
        with_temp_home(|| {
            ...
            assert!(jeikcode_config::schedule::load("rm-test-abc123").is_err());
        });
    }
    ...
    fn disable_unregisters_via_os_scheduler() {
        with_temp_home(|| {
            ...
            let loaded = jeikcode_config::schedule::load("dis-test-abc123").unwrap();
    ...
    fn enable_registers_via_os_scheduler() {
        with_temp_home(|| {
            ...
            let loaded = jeikcode_config::schedule::load("en-test-abc123").unwrap();
            assert!(loaded.enabled);
        });
    }
    ...
    fn sync_installs_enabled_and_uninstalls_disabled() {
        with_temp_home(|| {
            ...
            jeikcode_config::schedule::save(&t_on).unwrap();
            jeikcode_config::schedule::save(&t_off).unwrap();
