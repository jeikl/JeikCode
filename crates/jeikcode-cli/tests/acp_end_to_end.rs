...
//! (`jeikcode::acp::serve_over`) over one endpoint and a `Client.builder()` over
//! the other — the same handlers production uses, just over an in-memory pipe.
//!
//! The stub is `jeikcode_kernel::testkit::MockProvider`, the kernel's own
...
use jeikcode::acp::engine::EngineConfig;
use jeikcode::acp::{serve_over, AcpServeOptions};
use jeikcode_coding::{CodingAgentConfig, CodingProviderFactory, ProviderBuildError};
...
fn dummy_engine() -> EngineConfig {
    let mut config = jeikcode_coding::CodingAgentConfig::new(
        "test-key",
        "http://127.0.0.1:1",
        "stub-model",
        ".",
