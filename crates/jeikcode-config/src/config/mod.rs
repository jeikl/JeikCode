...
use jeikcode_telemetry::TelemetryConfig;
...
/// Append to the user `~/.jeikcode/config.toml`.
...
impl McpClientConfig {
    /// Load `[mcp]` from `~/.jeikcode/config.toml`, else defaults.
...
impl ToolTimeoutsConfig {
    ...
    /// Load `[tools.timeouts]` from `~/.jeikcode/config.toml`, else defaults.
    pub fn load_effective() -> Self {
        let path = Config::default_path();
        if path.is_file() {
            if let Ok(cfg) = Config::load(&path) {
...
impl Config {
    ...
    pub fn image_attach_support_for_model(&self, active_model: &str) -> ImageAttachSupport {
        // Prefer explicit config/protocol vision flags when the active model
        // resolves; fall back to name heuristics for runtime-local overrides.
        if let Ok(resolved) = self.resolve_model(Some(active_model)) {
...
fn project_legacy_model(account_id: &str, p: &ProviderConfig) -> ModelProfileConfig {
    ModelProfileConfig {
        ...
        reasoning_levels: p.reasoning_levels.clone(),
...
pub struct DatalogConfig {
    ...
    /// - `None` (or omitted) → `~/.jeikcode/datalog/` (default)
...
///
/// NOTE: `jeikcode-capabilities`' tool-registration gate contains an INTENTIONAL
/// DUPLICATE of this logic — it cannot call this helper because `jeikcode-config` is not
/// a dependency of the capabilities `tools` feature layer.  If you change the logic here
/// you MUST mirror the change in
/// `jeikcode-capabilities/src/tools/mod.rs` (the `request_user_input_on` block), and
/// vice versa.
...
/// (`~/.jeikcode/datalog`) is shown explicitly so users see exactly where
...
fn render_instructions_section() -> String {
    ...
    out.push_str("#\n");
    out.push_str("#   1. ~/.jeikcode/JEIKCODE.md           (global — your personal defaults)\n");
    out.push_str(
        "#   2. <project>/.jeikcode.md            (project — team-shared, commit to git)\n",
    );
    out.push_str("#      or <project>/JEIKCODE.md\n");
    out.push_str("#      or <project>/AGENTS.md           (AGENTS.md open standard)\n");
    out.push_str("#      or <project>/CLAUDE.md / claude.md (Claude Code compat)\n");
    out.push_str(
        "#   3. <project>/.jeikcode.user.md       (user — personal per-project, .gitignore)\n",
    );
    out.push_str("#\n");
...
fn render_hooks_json_section() -> String {
    ...
    out.push_str("#   ~/.jeikcode/hooks.json       (global hooks)\n");
...
impl Config {
    ...
    /// Resolve the config dir. Pure function for testability —
    /// `config_dir()` is a thin wrapper that injects real env + real home.
    fn resolve_config_dir(
        env_jeikcode_home: Option<String>,
        env_atomcode_home: Option<String>,
        home: Option<PathBuf>,
    ) -> PathBuf {
        if let Some(p) = env_jeikcode_home.filter(|s| !s.is_empty()) {
            return PathBuf::from(p);
        }
        if let Some(p) = env_atomcode_home.filter(|s| !s.is_empty()) {
            return PathBuf::from(p);
        }
        let home_dir = home.unwrap_or_else(|| PathBuf::from("."));
        let jeik_dir = home_dir.join(".jeikcode");
        if jeik_dir.exists() {
            return jeik_dir;
        }
...
"#,
    ...
    fn test_resolve_config_dir_uses_env_when_set() {
        ...
        assert_eq!(result, PathBuf::from("/tmp/custom-jeikcode-home"));
    ...
    fn test_resolve_config_dir_falls_back_to_home() {
        let result = Config::resolve_config_dir(None, None, Some(PathBuf::from("/Users/foo")));
        assert_eq!(result, PathBuf::from("/Users/foo/.jeikcode"));
    }

    #[test]
    fn test_resolve_config_dir_falls_back_to_dot_when_no_home() {
        let result = Config::resolve_config_dir(None, None, None);
        assert_eq!(result, PathBuf::from("./.jeikcode"));
    }
    ...
    fn render_datalog_section_with_dir_emits_real_value() {
        ...
        assert!(rendered.contains("dir = \"~/.jeikcode/logs\""));
    }
...
"#;
    ...
    #[test]
    fn flat_providers_project_to_individual_accounts() {
        let cfg: Config = serde_json::from_value(serde_json::json!({
            "providers": {
                "custom-glm": { "type": "openai", "base_url": "https://api.example.com/v1", "model": "GLM-5.2", "context_window": 64000 },
                "my-openai": { "type": "openai", "base_url": "https://api.openai.com/v1", "model": "gpt-4", "context_window": 128000 }
            }
        }))
        .unwrap();
        let accounts = cfg.logical_accounts();
        assert!(accounts.contains_key("custom-glm"));
        assert!(accounts.contains_key("my-openai"));

        let models = cfg.logical_models();
        assert_eq!(models["custom-glm"].account, "custom-glm");
