...
mod tests {
    ...
    fn non_https_and_custom_hosts_are_not_managed() {
        assert!(!is_managed_https_url("http://example.com"));
    ...
    fn fallback_requires_managed_uncapped_connect_failure() {
        // Without managed hosts configured, third party endpoints are never downgraded.
    ...
    fn fallback_gate_is_independent_of_which_failure_class_warrants_it() {
        ...
        assert!(!should_try_fallback("https://example.com", true, true));
