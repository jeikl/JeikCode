...
pub(super) fn zh_cn(msg: Msg<'_>) -> Cow<'static, str> {
    match msg {
        ...
        Msg::StatusWebuiHint =>
            "提示：使用 /webui 在浏览器中打开 JeikCode".into(),
...
"#.into(),
        ...
        Msg::WelcomeTipSession => "开始新会话".into(),
        ...
        Msg::HelpCustomCreateHint =>
            "  创建方式：~/.jeikcode/commands/<名称>.md 或 .jeikcode/commands/<名称>.md\n".into(),
...
Msg::PluginScopeUserDesc => "~/.jeikcode/plugins — 所有项目可见".into(),
Msg::PluginScopeProject => "为所有协作者安装（项目级）".into(),
Msg::PluginScopeProjectDesc => ".jeikcode/plugins — 通过 git 共享".into(),
Msg::PluginScopeLocal => "仅在本仓库为你安装（本地级）".into(),
Msg::PluginScopeLocalDesc => ".jeikcode/plugins/local — 不提交到 git".into(),
...
Msg::PluginActionBackDesc => "返回已安装插件列表".into(),
        ...
        Msg::PluginGitNotFound =>
            "💡 当前环境未安装 git 或 git 不在 PATH 中，插件市场自动安装和自动更新已禁用。请安装 git（macOS 可执行 `xcode-select --install`，Ubuntu 可执行 `sudo apt install git`）后重启 JeikCode。".into(),
...
Msg::CmdDescBackground => "在隔离的后台上下文中运行一次性任务（只读工具子集）".into(),
        ...
        Msg::CmdDescQuit => "退出 JeikCode".into(),
        ...
        Msg::CmdDescDesktop =>
            "打开 JeikCode 桌面端（已安装则启动，否则显示下载地址）".into(),
        Msg::DesktopOpening { name, path } =>
            format!("正在打开 {}…\n  {}\n", name, path).into(),
        Msg::DesktopNotInstalled { url } =>
            format!("未检测到 JeikCode 桌面端。下载安装：\n  {}\n", url).into(),
...
".into(),
        ...
        Msg::GoalHelp =>
            "  /goal — 朝着设定的条件自主进行多轮工作。\n  \
             ...
             说明：\n  \
             \u{20}\u{20}- 每轮由一个快速模型评估；通过 ~/.jeikcode/config.toml 中的 [providers] +\n  \
        ...
        Msg::AdminWarningBanner =>
            "\x1b[33m\u{26a0} 警告：正在以管理员权限运行。\n   模型可能可以访问系统文件。\n   建议改用普通权限、并在受限的工作目录中运行 JeikCode。\x1b[39m\n".into(),
        ...
        Msg::CliAboutServe => {
            "启动无界面 JeikCode 服务端，供远程 attach（Web UI + API）".into()
        }
        Msg::CliAboutAttach => {
            "连接正在运行的 JeikCode 服务端（打开 Web UI）".into()
        }
        ...
        Msg::CliHelpUninstallPurge => "完全清除 ~/.jeikcode/".into(),
        Msg::CliHelpUninstallKeepData => "完全保留 ~/.jeikcode/".into(),
        Msg::CliHelpUninstallDryRun => "仅打印计划；不执行操作".into(),
        Msg::CliHelpMcpGlobal => "写入 ~/.jeikcode/mcp.json 而非 <dir>/.mcp.json".into(),
