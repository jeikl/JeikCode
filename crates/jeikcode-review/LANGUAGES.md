...
1. Create `crates/jeikcode-review/rules/<name>.md` with the review checklist.
2. Register the file pattern in `crates/jeikcode-review/src/rules.rs` under `MATCHERS`.
3. Register the rule document under `RULE_DOCS` in the same file.
4. Run `cargo test -p jeikcode-review` to verify the matcher/doc consistency tests pass.
