...
//! [`run_to_completion`](jeikcode_kernel::agent::Agent::run_to_completion), and returns its
//! structured findings. Read-only ⇒ [`Safe`](jeikcode_kernel::tool::RiskLevel::Safe).
//!
...
//! signing gateway (the exact case `jeikcode-clix`'s `review` subcommand has to refuse).
...
use jeikcode_kernel::agent::{AutoRespond, ToolLoopPolicy};
...
use jeikcode_kernel::tool::{ProgressSink, Tool, ToolContext, ToolResult};
...
mod tests {
    ...
    async fn review_progress_hook_emits_round_activity() {
        ...
        let hook = ReviewProgressHook::new(jeikcode_kernel::tool::ProgressSink::new(Arc::new(
            move |message| {
                capture.lock().unwrap().push(message);
            },
        )));

        jeikcode_kernel::hook::LifecycleHooks::pre_request(
            &hook,
            &mut Vec::new(),
            &jeikcode_kernel::hook::TurnCtx {
                round: 3,
                max_rounds: None,
                ..Default::default()
            },
