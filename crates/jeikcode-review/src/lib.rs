...
//!    reviewer persona into a kernel [`Agent`](jeikcode_kernel::agent::Agent). It returns
...
//!
//! The DIFF to review is injected as the task by the caller (e.g. `jeikcode-clix`), so the
//! agent needs no shell to obtain it and stays strictly read-only.
//!
//! ```no_run
//! # async fn demo() -> Result<(), String> {
//! use jeikcode_review::{build_review_agent, ReviewAgentConfig};
...
/// `jeikcode-capabilities` directly.
pub use jeikcode_capabilities::tools::{Finding, ReportFindingTool};
