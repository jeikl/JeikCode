...
use jeikcode_capabilities::codeintel::lang::Lang;
...
/// agent. Used by `jeikcode-clix`'s `review` driver so the FIRST review pass and the
...
///
/// Exposed so drivers (e.g. `jeikcode-clix`) don't need a direct `tokio-util` dep just
...
fn mount_review_tools(
    ...
    if !skill_dirs.is_empty() {
        let skills = Arc::new(jeikcode_capabilities::skills::SkillRegistry::load(
            skill_dirs,
        ));
        jeikcode_capabilities::skills::register_skill_tools(&mut reg, skills);
        names.extend_from_slice(jeikcode_capabilities::skills::skill_tool_names());
    }
    reg.mount(&names)
}
...
mod tests {
    ...
    impl LlmProvider for ScriptedReviewProvider {
        ...
        async fn chat_stream(
            ...
            let has_tool_result = messages
                .iter()
                .any(|m| matches!(m.role, jeikcode_kernel::message::Role::Tool));
    ...
    async fn max_rounds_stops_runaway_review() {
        ...
        assert_eq!(
            outcome.stop,
            jeikcode_kernel::event::StopReason::MaxRounds,
    ...
    async fn max_turn_duration_stops_stalled_review() {
        ...
        assert_eq!(
            outcome.stop,
            jeikcode_kernel::event::StopReason::Cancelled,
    ...
    fn graph_tools_gated_by_mount_flag() {
        ...
        let graph_names = jeikcode_capabilities::codeintel::codeintel_tool_names();
        assert!(
