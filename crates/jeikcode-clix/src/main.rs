...
struct ReviewArgs {
    ...
    /// Config file path (default: ~/.jeikcode/config.toml).
    #[arg(long)]
...
async fn review(args: ReviewArgs) -> Result<()> {
    ...
    let (task, trace_label) = match custom_task {
        ...
        None => {
            ...
            changed_files = jeikcode_review::changed_files_from_diff(&diff);
            // Language rules matched against the changed files.
            if !args.no_rules {
                let section = jeikcode_review::render_rules_section(
                    &changed_files,
            ...
            let impact_plan = jeikcode_review::render_review_impact_plan(&diff);
            let diff = jeikcode_review::annotate_diff_line_numbers(&diff);
            let t = format!(
...
struct ReviewRun {
    ...
    error: Option<String>,
    /// Final token usage.
    usage: Option<jeikcode_kernel::stream::TokenUsage>,
...
/// A `[providers.<name>]` entry in `~/.jeikcode/config.toml`. All fields optional so a
...
pub(crate) struct ConfigSelection {
    ...
    pub(crate) language: Option<jeikcode_config::locale::Locale>,
}
...
fn default_config_path() -> Option<PathBuf> {
    ...
    Some(PathBuf::from(home).join(".jeikcode").join("config.toml"))
}
...
fn uncovered_files(changed_files: &[String], findings: &[Finding]) -> Vec<String> {
    ...
    let mut files: Vec<String> = changed_files
        .iter()
        .filter(|c| !jeikcode_review::is_low_signal_file(c))
...
/// [`changed_files_from_diff`](jeikcode_review::changed_files_from_diff), i.e. `b/`-stripped).
...
--- a/pkg/b.go\n+++ b/pkg/b.go\n@@ -1 +1 @@\n-old\n+newB\n";
    ...
    fn json_envelope_carries_findings_text_usage() {
        ...
        let usage = jeikcode_kernel::stream::TokenUsage {
            prompt: 10,
            completion: 5,
            cached: 2,
        };
...
"#;
    ...
    fn selected_config_carries_top_level_language() {
        ...
        assert_eq!(
            selected.language,
            Some(jeikcode_config::locale::Locale::ZhCn)
        );
