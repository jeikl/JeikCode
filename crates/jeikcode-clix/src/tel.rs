...
use jeikcode_telemetry::config::{resolve, CliOverride, ProcessEnv, TelemetryConfig};
...
fn atomcode_dir() -> PathBuf {
    ...
    match std::env::var_os("HOME").or_else(|| std::env::var_os("USERPROFILE")) {
        Some(h) => PathBuf::from(h).join(".jeikcode"),
        None => PathBuf::from(".jeikcode"),
    }
}
...
/// Read the `[telemetry]` section from the config file (default `~/.jeikcode/config.toml`,
/// or `config_override`). Any read/parse failure → defaults — telemetry is non-fatal, exactly
/// as jeikcode-cli treats it (a broken config must not break `review`/`code`).
...
pub fn meter_provider(
    ...
    Arc::new(
        jeikcode_coding::telemetry::MeteredProvider::new(
            inner,
            sink.clone(),
            "openai",
            base_url,
...
/// `jeikcode_review::build_review_agent`'s internal construction, but lets us wrap it with
/// telemetry before handing it to `build_review_agent_with`).
pub fn build_review_provider(
    cfg: &jeikcode_review::ReviewAgentConfig,
    ...
    // `jeikcode_review::build_review_agent`), not the adapter's hardcoded 120s.
