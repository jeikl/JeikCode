...
#[async_trait]
impl jeikcode_kernel::provider::LlmProvider for HangingOpenProvider {
...
async fn cancel_aborts_a_turn_hung_in_the_stream_open() {
    let mut handle = jeikcode_kernel::agent::Agent::builder()
...
#[async_trait]
impl jeikcode_kernel::middleware::ToolMiddleware for GateMiddleware {
    async fn before(
        &self,
        call: &mut ToolCall,
        _tool: &Arc<dyn Tool>,
        rt: &jeikcode_kernel::request::RequestCtx,
    ) -> jeikcode_kernel::middleware::BeforeOutcome {
        let v = rt
            .request("approval", serde_json::json!({"tool": call.name}))
            .await;
        match v.as_str() {
            Some("allow") => jeikcode_kernel::middleware::BeforeOutcome::Proceed,
            _ => jeikcode_kernel::middleware::BeforeOutcome::deny("denied"),
        }
    }
}
...
async fn cancel_unblocks_pending_middleware_request() {
    ...
    assert_eq!(
        reason,
        Some(jeikcode_kernel::event::StopReason::Cancelled),
