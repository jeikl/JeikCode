...
use jeikcode_kernel::checkpoint::{CompactionCheckpoint, CompactionCheckpointError};
use jeikcode_kernel::event::{AgentCommand, AgentEvent};
use jeikcode_kernel::message::{Role, SessionSnapshot};
use jeikcode_kernel::stream::{StreamEvent, TokenUsage};
use jeikcode_kernel::testkit::{
...
async fn drive_turn_collect(
    handle: &mut jeikcode_kernel::agent::AgentHandle,
    text: &str,
) -> Vec<AgentEvent> {
    handle
        .commands
...
async fn snapshot(handle: &mut jeikcode_kernel::agent::AgentHandle) -> SessionSnapshot {
...
async fn replaceability_two_strategies_differ() {
    ...
    async fn run(
        strategy: Arc<dyn jeikcode_kernel::message::CompactionStrategy>,
    ) -> SessionSnapshot {
        let provider =
    ...
    let baseline = run(Arc::new(jeikcode_kernel::message::NoCompaction)).await;
...
async fn mid_turn_snapshot_is_queued_and_delivered_after_turn() {
    use jeikcode_kernel::testkit::DeferredCommands;
