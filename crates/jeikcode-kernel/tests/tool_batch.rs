...
fn tc(id: &str, name: &str) -> jeikcode_kernel::tool::ToolCall {
    jeikcode_kernel::tool::ToolCall {
        id: id.into(),
        name: name.into(),
        arguments: "{}".into(),
    }
...
async fn approval_requests_are_sequential_not_concurrent() {
    ...
    #[async_trait::async_trait]
    impl jeikcode_kernel::tool::Tool for RiskyWriteTool2 {
        fn name(&self) -> &str {
            "risky_write2"
        }
        fn description(&self) -> &str {
        ...
        fn risk(&self, _args: &str) -> jeikcode_kernel::tool::RiskLevel {
            jeikcode_kernel::tool::RiskLevel::Risky
        }
...
async fn read_only_bash_shaped_calls_overlap() {
    ...
    let mk = |id: &str, name: &str| {
        StreamEvent::ToolCall(jeikcode_kernel::tool::ToolCall {
            id: id.into(),
            name: name.into(),
...
async fn non_read_only_bash_shaped_call_serializes() {
    ...
    let wr = StreamEvent::ToolCall(jeikcode_kernel::tool::ToolCall {
        id: "2".into(),
        name: "w".into(),
...
fn tc_args(id: &str, name: &str, args: &str) -> jeikcode_kernel::tool::ToolCall {
    jeikcode_kernel::tool::ToolCall {
        id: id.into(),
        name: name.into(),
        arguments: args.into(),
