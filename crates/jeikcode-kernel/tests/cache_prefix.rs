...
use jeikcode_kernel::testkit::{DangerousBashTool, EchoTool, RecordingProvider};
use jeikcode_kernel::tool::{ToolCall, ToolDef, ToolRegistry};
use std::sync::Arc;
...
fn agent_handle(provider: Arc<RecordingProvider>) -> jeikcode_kernel::agent::AgentHandle {
    let mut reg = ToolRegistry::new();
    reg.register(Arc::new(EchoTool));
    reg.register(Arc::new(DangerousBashTool));
    jeikcode_kernel::agent::Agent::builder()
        .provider(provider)
...
async fn drive_one_turn(handle: &mut jeikcode_kernel::agent::AgentHandle, text: &str) {
    handle
        .commands
