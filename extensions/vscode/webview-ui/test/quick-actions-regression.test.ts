...
function testQuickActionPromptsFollowChineseLocale() {
  assert.equal(getQuickActionPrompt('intro', 'zh-cn'), '介绍一下 JeikCode 可以帮我做什么。');
  ...
  assert.equal(getQuickActionPrompt('configuration', 'zh-cn'), '请帮我检查 JeikCode、模型和 Provider 配置，并说明如何配置。');
  assert.equal(getQuickActionPrompt('tips', 'zh-cn'), '请介绍在 VS Code 中使用 JeikCode 的常用工作流和技巧。');
}

function testQuickActionPromptsKeepEnglishFallback() {
  assert.equal(getQuickActionPrompt('intro', 'en-US'), 'Introduce what JeikCode can help me do.');
