...
class JeikCodeDaemonProcessTest {
    @Test
    fun locatesBundledDaemonResource() {
        val resolution = JeikCodeDaemonProcess(JeikCodeSettings()).locateBinary()

        assertNotNull(resolution)
        assertEquals(emptyList(), resolution.argsPrefix)
        assertTrue(resolution.path.contains("jeikcode-jetbrains"))
        assertTrue(resolution.path.endsWith(if (isWindows()) "jeikcode-daemon.exe" else "jeikcode-daemon"))
    }

    @Test
    fun readsExpectedBundledVersion() {
        val version = JeikCodeDaemonProcess(JeikCodeSettings()).expectedBundledVersion()
    ...
    fun extractsBundledDaemonToContentAddressedPath() {
        val daemon = JeikCodeDaemonProcess(JeikCodeSettings())
