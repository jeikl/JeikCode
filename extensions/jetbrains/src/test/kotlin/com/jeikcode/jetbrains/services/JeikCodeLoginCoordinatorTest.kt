...
class JeikCodeLoginCoordinatorTest {
    @Test
    fun `concurrent callers share one application login attempt`() {
        ...
        val coordinator = JeikCodeLoginCoordinator()
        ...
        assertEquals(listOf("Opened browser for JeikCode sign-in."), secondStatuses)
    ...
    fun `typed terminal state completes exceptionally instead of authorizing`() {
        ...
        val error = runCatching {
            JeikCodeLoginCoordinator().login(transport) {}.get(1, TimeUnit.SECONDS)
        }.exceptionOrNull()
