...
class JeikCodeLoginCoordinator {
    private val attempts = mutableMapOf<String, LoginAttempt>()
    private val logger = Logger.getInstance(JeikCodeLoginCoordinator::class.java)
    ...
    internal fun login(
        ...
        transport.start()
            .thenCompose { start ->
                attempt.publish("Opened browser for JeikCode sign-in.")
    ...
    private inner class LoginAttempt(
        ...
        private fun notifyListener(listener: (String) -> Unit, status: String) {
            try {
                ...
                logger.warn("JeikCode login status listener failed", error)
            }
        }
    }
    ...
    companion object {
        fun getInstance(): JeikCodeLoginCoordinator =
            ApplicationManager.getApplication().getService(JeikCodeLoginCoordinator::class.java)
    }
}
