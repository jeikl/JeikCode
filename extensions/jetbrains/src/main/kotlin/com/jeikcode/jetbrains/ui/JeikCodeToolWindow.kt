...
const val JEIKCODE_TOOL_WINDOW_ID = "JeikCode"
private val JEIKCODE_TAB_ID_KEY = Key.create<String>("jeikcode.tabId")
private val JEIKCODE_TOOL_WINDOW_MIN_SIZE = Dimension(360, 300)

fun createJeikCodeChatContent(project: Project, toolWindow: ToolWindow, closeable: Boolean): JeikCodeChatPanel {
    val name = nextChatTabName(toolWindow)
    val runtime = SessionWorkspace.getInstance(project).createRuntime(name)
    return createJeikCodeChatContent(project, toolWindow, closeable, runtime, name)
}

fun restoreJeikCodeChatContent(project: Project, toolWindow: ToolWindow, tab: WorkspaceTabState): JeikCodeChatPanel {
    ...
    return createJeikCodeChatContent(project, toolWindow, closeable = true, runtime, name)
}

private fun createJeikCodeChatContent(
    project: Project,
    ...
    val panel = JeikCodeChatPanel(project, runtime)
    val content = ContentFactory.getInstance().createContent(panel, name, false).apply {
        isCloseable = closeable
        description = "JeikCode Chat"
        putUserData(JEIKCODE_TAB_ID_KEY, runtime.tabId)
        setDisposer(panel)
    }
    toolWindow.contentManager.addContent(content)
    toolWindow.contentManager.setSelectedContent(content)
    toolWindow.component.minimumSize = JEIKCODE_TOOL_WINDOW_MIN_SIZE
...
fun selectedJeikCodeChatPanel(project: Project): JeikCodeChatPanel? {
    val toolWindow = ToolWindowManager.getInstance(project).getToolWindow(JEIKCODE_TOOL_WINDOW_ID) ?: return null
    val selected = toolWindow.contentManager.selectedContent?.component as? JeikCodeChatPanel
    if (selected != null) return selected
    return toolWindow.contentManager.contents
        .asSequence()
        .mapNotNull { it.component as? JeikCodeChatPanel }
        .firstOrNull()
}

fun ensureJeikCodeChatContent(project: Project, toolWindow: ToolWindow): JeikCodeChatPanel {
    val selected = toolWindow.contentManager.selectedContent?.component as? JeikCodeChatPanel
    if (selected != null) return selected
    ...
    return createJeikCodeChatContent(project, toolWindow, closeable = true)
}

fun openJeikCodeChatTab(project: Project, newTab: Boolean = false, focusInput: Boolean = true) {
    ApplicationManager.getApplication().invokeLater {
        val toolWindow = ToolWindowManager.getInstance(project).getToolWindow(JEIKCODE_TOOL_WINDOW_ID) ?: return@invokeLater
        toolWindow.show()
        val panel = if (newTab) {
            createJeikCodeChatContent(project, toolWindow, closeable = true)
        } else {
            ensureJeikCodeChatContent(project, toolWindow)
        }
        if (focusInput) panel.focusInput()
    }
}

fun openJeikCodeWelcomePage(project: Project) {
    ApplicationManager.getApplication().invokeLater {
        val toolWindow = ToolWindowManager.getInstance(project).getToolWindow(JEIKCODE_TOOL_WINDOW_ID) ?: return@invokeLater
        toolWindow.show()
        ensureJeikCodeChatContent(project, toolWindow).showWelcomePage()
    }
}

fun openJeikCodeSessionHistory(project: Project) {
    ApplicationManager.getApplication().invokeLater {
        val toolWindow = ToolWindowManager.getInstance(project).getToolWindow(JEIKCODE_TOOL_WINDOW_ID) ?: return@invokeLater
        toolWindow.show()
        ensureJeikCodeChatContent(project, toolWindow).showSessionHistory()
    }
}
...
fun closeCurrentChatTab(project: Project) {
    ApplicationManager.getApplication().invokeLater {
        ...
        if (contentManager.contentCount <= 1) {
            // 最后一个标签页：清空内容
            val panel = selected.component as? JeikCodeChatPanel
...
fun contentTabId(content: Content): String? =
    content.getUserData(JEIKCODE_TAB_ID_KEY)

fun updateJeikCodeChatTabTitle(project: Project, panel: JeikCodeChatPanel, title: String) {
    val normalizedTitle = title.trim()
    if (normalizedTitle.isEmpty()) return
    val toolWindow = ToolWindowManager.getInstance(project).getToolWindow(JEIKCODE_TOOL_WINDOW_ID) ?: return
...
private fun installTabPopupMenu(toolWindow: ToolWindow, project: Project) {
    SwingUtilities.invokeLater {
        ...
        // 避免重复安装
        if (tabPane.getClientProperty("jeikcode-popup-installed") == true) return@invokeLater
        tabPane.putClientProperty("jeikcode-popup-installed", true)
        ...
        tabPane.addMouseListener(object : MouseAdapter() {
            ...
            private fun maybeShowPopup(e: MouseEvent) {
                ...
                menu.add(JMenuItem("关闭标签页").apply {
                    addActionListener {
                        if (contentManager.contentCount <= 1) {
                            val panel = clickedContent.component as? JeikCodeChatPanel
                            panel?.startNewConversation()
                        } else {
                ...
                menu.add(JMenuItem("新建标签页").apply {
                    addActionListener { openJeikCodeChatTab(project, newTab = true) }
                })
...
private fun installContentSelectionListener(toolWindow: ToolWindow, project: Project) {
    if (toolWindow.component.getClientProperty("jeikcode-content-listener-installed") == true) return
    toolWindow.component.putClientProperty("jeikcode-content-listener-installed", true)
