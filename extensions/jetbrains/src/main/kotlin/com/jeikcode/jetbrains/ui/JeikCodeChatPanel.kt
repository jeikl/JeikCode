...
import com.jeikcode.jetbrains.diagnostics.JeikCodeDiagnostics
import com.jeikcode.jetbrains.actions.openJeikCodeSettings
...
import com.jeikcode.jetbrains.settings.JeikCodeContextLevel
...
private const val JEIKCODE_DOCS_ZH_URL = "https://jeikcode.github.com/docs/zh/index.html"
private const val JEIKCODE_DOCS_EN_URL = "https://jeikcode.github.com/docs/en/index.html"
...
class JeikCodeChatPanel(
    ...
    private fun currentDocsUrl(): String =
        if (welcomeLanguage == "zh") JEIKCODE_DOCS_ZH_URL else JEIKCODE_DOCS_EN_URL
    ...
    private fun showCreateProviderDialog() {
        ...
        val choice = JOptionPane.showConfirmDialog(this, form, "Create JeikCode Provider", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE)
        ...
        if (request.name.isBlank() || request.type.isBlank() || request.model.isBlank()) {
            Messages.showWarningDialog(this, "Name, type, and model are required.", "JeikCode"); return
        }
    ...
    private fun showEditProviderDialog() {
        ...
        val choice = JOptionPane.showConfirmDialog(this, form, "Edit JeikCode Provider", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE)
        if (choice != JOptionPane.OK_OPTION) return
    ...
    private fun deleteSelectedProvider() {
        val selected = selectedProvider() ?: return
        val choice = Messages.showYesNoDialog(this, "Delete provider \"${selected.name}\" from JeikCode config?", "JeikCode", Messages.getWarningIcon())
    ...
    private fun showThinkingDialog() {
        ...
        val choice = JOptionPane.showConfirmDialog(this, form, "JeikCode Thinking - ${selected.name}", JOptionPane.OK_CANCEL_OPTION, JOptionPane.PLAIN_MESSAGE)
        if (choice != JOptionPane.OK_OPTION) return

        val budgetValue = budget.text.trim().takeIf { it.isNotBlank() }?.toIntOrNull()
        if (budget.text.trim().isNotBlank() && budgetValue == null) { Messages.showWarningDialog(this, "Thinking budget must be a number.", "JeikCode"); return }
    ...
    private fun openSessionHistoryDialog(initialSessions: List<SessionMeta>) {
        ...
        val dialog = JDialog(SwingUtilities.getWindowAncestor(this), "JeikCode Session History", Dialog.ModalityType.APPLICATION_MODAL).apply {
            contentPane = panel; pack(); setLocationRelativeTo(this@JeikCodeChatPanel)
        }
        ...
        rename.addActionListener {
            ...
            if (nextName.isBlank()) { Messages.showWarningDialog(dialog, "Session name cannot be empty.", "JeikCode"); return@addActionListener }
        ...
        delete.addActionListener {
            ...
            val label = if (selected.size == 1) "Delete JeikCode session \"${selected.first().displayName}\" from local history?" else "Delete ${selected.size} JeikCode sessions from local history?"
            val choice = Messages.showYesNoDialog(dialog, label, "JeikCode", Messages.getWarningIcon())
    ...
    private fun renameSelectedSession() {
        ...
        if (nextName.isBlank()) { Messages.showWarningDialog(this, "Session name cannot be empty.", "JeikCode"); return }
    ...
    private fun deleteSelectedSession() {
        val selected = sessionPicker.selectedItem as? SessionMeta ?: return
        val choice = Messages.showYesNoDialog(this, "Delete JeikCode session \"${selected.displayName}\" from local history?", "JeikCode", Messages.getWarningIcon())
        if (choice != Messages.YES) return
    ...
    private fun showDiagnostics() {
        ...
        val text = JeikCodeDiagnostics.summary(project, details)
        CopyPasteManager.getInstance().setContents(StringSelection(text))
        val area = JTextArea(text).apply { isEditable = false; lineWrap = false; rows = 22; columns = 72 }
        JOptionPane.showMessageDialog(this, JScrollPane(area), "JeikCode Diagnostics (copied)", JOptionPane.INFORMATION_MESSAGE)
    }
    ...
    private fun loadSession(meta: SessionMeta) {
        service.loadSessionDetail(meta).whenComplete { detail, error ->
            SwingUtilities.invokeLater {
                ...
                updateJeikCodeChatTabTitle(project, this@JeikCodeChatPanel, detail.name.ifBlank { detail.id.take(8) })
                persistRuntimeSession()
    ...
    private fun applyLastCodeBlock() {
        ...
        if (code.isNullOrBlank()) { Messages.showWarningDialog(project, "No code block found in the last JeikCode response.", "JeikCode"); return }
        val editor = FileEditorManager.getInstance(project).selectedTextEditor
        if (editor == null) { Messages.showWarningDialog(project, "Open an editor file before applying code.", "JeikCode"); return }
        ...
        val request = SimpleDiffRequest("JeikCode Apply Code Preview", contentFactory.create(before), contentFactory.create(after), "Current editor", "After JeikCode")
        DiffManager.getInstance().showDiff(project, request)
        val choice = Messages.showYesNoDialog(project, "Apply the previewed JeikCode code block to the active editor?", "JeikCode", Messages.getQuestionIcon())
        if (choice != Messages.YES) { addSystemMessage("Apply Code cancelled after preview."); return }
        WriteCommandAction.runWriteCommandAction(project, "Apply JeikCode Code", null, Runnable {
    ...
    private fun requestPermissionDecision(event: ChatEvent.PermissionRequest) {
        ...
        SwingUtilities.invokeLater {
            ...
            val message = buildString {
                appendLine("JeikCode wants to run a tool."); appendLine()
            ...
            val choice = Messages.showDialog(
                this, message, "JeikCode Tool Permission",
    ...
    private fun updateCurrentSessionTitle(sessionId: String, title: String) {
        ...
        updateJeikCodeChatTabTitle(project, this, normalizedTitle)
        persistRuntimeSession()
    }
    ...
    private fun buildAutomaticContext(existingContext: List<ChatContextItem>): List<ChatContextItem> {
        ...
        if (level == JeikCodeContextLevel.Minimal) return emptyList()
        val result = mutableListOf<ChatContextItem>()
        if (level == JeikCodeContextLevel.ProjectContext) {
            result += ChatContextItem(path = project.basePath.orEmpty(), displayName = "Project context", language = "text", content = buildString {
                ...
                currentSession?.id?.let { appendLine("JeikCode session: $it") }
    ...
    private fun chooseFilesForContext() {
        val descriptor = FileChooserDescriptor(true, false, false, false, false, true).withTitle("Attach Files to JeikCode")
    ...
    private fun attachVirtualFile(file: VirtualFile) {
        val path = file.path
        when (SensitivePathClassifier.classify(path)) {
            PathSensitivity.Block -> { Messages.showWarningDialog(project, "JeikCode will not attach this sensitive file.", "JeikCode"); return }
            PathSensitivity.StrongConfirm -> {
                val choice = Messages.showYesNoDialog(project, "This file may contain sensitive information. Attach it to the next JeikCode message?", "JeikCode", Messages.getWarningIcon())
                if (choice != Messages.YES) return
            }
            PathSensitivity.Warn, PathSensitivity.Normal -> Unit
        }
        ...
        val content = try { String(file.contentsToByteArray(), Charsets.UTF_8) } catch (error: Exception) { Messages.showWarningDialog(project, "Could not read ${file.name}: ${error.message}", "JeikCode"); return }
        if (content.isBlank()) return
        if (content.length > MAX_ATTACHED_FILE_CHARS) { Messages.showWarningDialog(project, "This file is too large to attach. Select a smaller file or attach a selection.", "JeikCode"); return }
        val relative = project.basePath?.let { base -> if (path.startsWith(base)) path.removePrefix(base).trimStart('/', '\\') else path } ?: path
    ...
    private fun attachImageFile(file: VirtualFile, mediaType: String) {
        val bytes = try {
            ...
            Messages.showWarningDialog(project, "Could not read ${file.name}: ${error.message}", "JeikCode")
            return
        }
    ...
    internal fun showGearMenu() {
        ...
        menu.add(JMenuItem(labels.settings).apply { addActionListener { project.openJeikCodeSettings() } })
