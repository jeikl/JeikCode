...
import com.jeikcode.jetbrains.daemon.JeikCodeDaemonProcess
...
internal data class DaemonEndpointKey(
    ...
    companion object {
        fun from(settings: JeikCodeSettings): DaemonEndpointKey = DaemonEndpointKey(
            host = settings.host,
            port = settings.port,
        )
    }
...
internal data class DaemonConnectionKey(
    ...
    companion object {
        fun from(settings: JeikCodeSettings): DaemonConnectionKey = DaemonConnectionKey(
...
internal fun interface DaemonControlFactory {
    fun create(settings: JeikCodeSettings, timeoutMs: Int, auth: DaemonAuth): DaemonControl
}

internal fun interface DaemonProcessFactory {
    fun create(settings: JeikCodeSettings): DaemonProcessLauncher
}
...
internal class DaemonSupervisorEngine(
    ...
    fun ensureReady(settings: JeikCodeSettings, auth: DaemonAuth): CompletableFuture<DaemonReady> {
        ...
        val shared = synchronized(lock) {
            if (disposed) {
                return failed(ConnectionErrorKind.StartFailed, "JeikCode daemon supervisor is disposed.")
            }
            ...
            .thenCompose { attempt ->
                ...
                when {
                    ...
                    !settings.autoStart -> failed(
                        ...
                        buildString {
                            append("JeikCode daemon is not running and auto-start is disabled.")
    ...
    private fun acceptOrRestart(
        ...
        if (health.service != "jeikcode-daemon") {
            return failed(
                ConnectionErrorKind.PortUsedByNonJeikCode,
    ...
    private fun launchAndAwait(
        ...
        when (result) {
            DaemonLaunchResult.MissingBinary -> failed(
                ...
                "JeikCode CLI or bundled daemon was not found.",
            )
            is DaemonLaunchResult.Failed -> failed(
                ConnectionErrorKind.StartFailed,
                "Failed to start JeikCode daemon: ${result.message}",
            )
            is DaemonLaunchResult.Started -> {
                ...
                if (process == null) {
                    return@thenCompose failed(
                        ConnectionErrorKind.StartFailed,
                        "JeikCode daemon supervisor was disposed during startup.",
                    )
                }
                ...
                when {
                    ...
                    nanoTime() >= deadlineNanos -> {
                        ...
                        failed(
                            ...
                            buildString {
                                append(
                                    "JeikCode daemon did not become ready within " +
        ...
        return control.health()
            ...
            .thenCompose { health ->
                if (health?.service == "jeikcode-daemon" && !expectation.mismatches(health)) {
                    ...
                    failed(
                        ConnectionErrorKind.StartFailed,
                        "JeikCode daemon exited with code ${exit.exitCode}: $detail",
                    )
                }
            }
    }
...
private data class DaemonExpectation(
    ...
    fun mismatchMessage(health: HealthResponse): String =
        if (version != null && health.version != version) {
            "JeikCode daemon version mismatch: running ${health.version}, expected $version."
        } else {
            "JeikCode daemon binary does not match the bundled build."
        }
}

@Service(Service.Level.APP)
class JeikCodeDaemonSupervisor : Disposable {
    private val engine = DaemonSupervisorEngine(
        controlFactory = DaemonControlFactory { settings, timeoutMs, auth ->
            val client = JeikCodeDaemonClient(settings.host, settings.port, timeoutMs, auth)
        ...
        processFactory = DaemonProcessFactory(::JeikCodeDaemonProcess),
    )

    internal fun ensureReady(settings: JeikCodeSettings, auth: DaemonAuth): CompletableFuture<DaemonReady> =
        engine.ensureReady(settings, auth)

    override fun dispose() = engine.dispose()

    companion object {
        fun getInstance(): JeikCodeDaemonSupervisor =
            ApplicationManager.getApplication().getService(JeikCodeDaemonSupervisor::class.java)
    }
}
